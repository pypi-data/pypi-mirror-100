...
from image_restoration_tools.network import Net, SRN
...
for i in sys.path:
        if i[-8:]=='packages':
                if 'image_restoration_tools' in os.listdir(i):
                        syspath = i
...
class Agent:
    ...
    def restore_image(self, img, path_length=1, isNOISE=False, isJPEG=False, isBLUR=False):
        # 预处理
        if isNOISE:
            img = self.step(img, 0) 
            path_length = 1
        ...
        for _ in range(path_length): 
            ...
            if obs[0] > 2 and tool0:
                ...
                tool = 1
                # 先去运动模糊，而后sharpen
                img_E = self.step(img, 4) 
                img_E = self.step(img_E, 3) 
...
class Tool:
    def __init__(self):
        ...
        self.motion_deblur = self.srn()
        
        
    def net(self, ck):
        model = Net()
        model.load_state_dict(torch.load(syspath + "/image_restoration_tools/models/" + ck,
    ...
    def srn(self):
        model = SRN(syspath + "/image_restoration_tools/models/srn/")
        return model
    ...
    def step(self, img, type):
        ...
        if type == 0: 
            ...
            if len(img_E.shape) == 3:
                ...
                img_E = np.transpose(img_E, (0, 2, 1, 3))
            else:
                img_E = np.transpose(img_E, (0, 2, 3, 1))
            img_E = self.motion_deblur.test(img_E)
            img_E = np.transpose(img_E, (1, 0, 2))
