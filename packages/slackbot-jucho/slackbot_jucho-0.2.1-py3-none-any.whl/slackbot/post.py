...
def post_message(message):
    ...
    data = [{"type": "section",
         "text": {
              "type": "mrkdwn",
              "text": f"<@U01SH0HL4PR|cal>, {message}"
          }}]
...
#             "user": "U01SH0HL4PR",
             "text": {
                 "type": "mrkdwn",
                 "text": f"<@U01SH0HL4PR|cal>, the work {work_name} done at {datetime.now().strftime(r'%m%d_%H%M')}"
             }}]
    slack_data = json.dumps({'text': f"[COMPLETED] {work_name} has been completed :)",
        'blocks': data})
    response = requests.post(webhook_url,
                             data=slack_data,
                             headers={'Content-type': 'application/json'}
