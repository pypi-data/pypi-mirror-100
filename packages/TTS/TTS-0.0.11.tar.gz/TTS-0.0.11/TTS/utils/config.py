...
class BaseTrainingConfig(Coqpit):
    ...
    # dataset
    datasets: Union[List[BaseDatasetConfig], BaseDatasetConfig] = None
    # distributed
    ...
    #     '''Check config fields'''
    #     c = asdict(self)
        # check_argument('model', c, restricted=True)
        # check_argument('run_name', c, restricted=True)
        # check_argument('run_description', c)
        # # training params
        # check_argument('batch_size', c, min_val=1, restricted=True)
        # check_argument('batch_group_size', c, min_val=0, restricted=True)
        # check_argument('eval_batch_size', c, restricted=True, min_val=1)
        # check_argument('mixed_precision', c, restricted=True)
        # check_argument('loss_masking', c, restricted=True)
        # # eval params
        # check_argument('run_eval', c, restricted=True)
        # check_argument('test_delay_epochs', c, restricted=True, min_val=0)
        # check_argument('print_eval', c, restricted=False)
        # # logging
        # check_argument('print_step', c, restricted=True, min_val=1)
        # check_argument('tb_plot_step', c, restricted=True, min_val=1)
        # check_argument('save_step', c, restricted=True, min_val=1)
        # check_argument('checkpoint', c, restricted=True)
        # check_argument('tb_model_param_stats', c, restricted=True)
        # check_argument('keep_all_best', c, restricted=False)
        # check_argument('keep_after', c, restricted=False)
        # # data loading
        # check_argument('num_loader_workers', c, restricted=True, min_val=0)
        # check_argument('num_val_loader_workers', c, restricted=True, min_val=0)
        # check_argument('batch_group_size', c, restricted=True, min_val=0)
        # check_argument('min_seq_len', c, restricted=True, min_val=0)
        # check_argument('max_seq_len', c, restricted=True, min_val=10)
        # check_argument('compute_f0', c, restricted=False)
        # check_argument('use_noise_augment', c, restricted=False)
        # # paths
        # check_argument('output_path', c, restricted=True)
        # check_argument('phoneme_cache_path', c, restricted=True)
        # check_argument('datasets', c, restricted=True)
