...
class PoleTracking():
    ...
    def crop_image_from_top_left(im, crop_top_left, size_crop, inflation = 1):
        ...
        c_cropped = im[crop_top_left[0]:crop_bottom_right[0],crop_top_left[1]:crop_bottom_right[1], ...].astype('uint8')
        ...
        if len(crop_dim)>2: # crop 3D or more
          cropped_image[TL_adj[0]:TL_adj[0]+c_cropped.shape[0], TL_adj[1]:TL_adj[1]+c_cropped.shape[1], ...] = c_cropped
        else:# crop 2D
          cropped_image[TL_adj[0]:TL_adj[0]+c_cropped.shape[0], TL_adj[1]:TL_adj[1]+c_cropped.shape[1]] = c_cropped
    ...
    def get_trial_and_file_names(self, print_them = False, num_to_print = None):
        trial_sep = [k for k in self.video_files[0] if k in '-_'][-1] # '-_' are the possible seperators 
    ...
    def track(self, video_file, match_method = 'cv2.TM_CCOEFF'):
        ...
        while success:
          # preprocess image
          if 'frame' in locals():
            frame, crop_top_left, crop_bottom_right = self.crop_image_from_top_left(og_frame, top_left+crop_top_left, [w, h], 3)
          else:
            frame = og_frame 
    ...
    def cut_out_pole_template(self, video_directory, crop_size, frame_num = 2000, file_ind = None):
      ...
      while str(top_left).lower() != 'exit':
        try:
          ...
          plt.draw()
          plt.pause(0.01)
          top_left = input('Input the pole center coordinates seperated by a comma \ny (0 = top, max = bottom) \nx (0 = left, max = right) \nor enter "EXIT" to finalize image selection'+'\nlast selection --> ' +str(U_in_2_print[0])+ ' , ' + str(U_in_2_print[1])+ '\n')
        except:
          print('oops try again enter a pair of numbers or enter exit to finish select... or maybe you entered an out of range pair of numbers')
