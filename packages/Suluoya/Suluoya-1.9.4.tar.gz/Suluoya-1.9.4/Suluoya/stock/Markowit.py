...
try:
    from ..chart.svg_charts import scatter
    from ..chart.svg_charts import pie
    from ..chart.svg_charts import line
    ...
    def __init__(self, names=['贵州茅台', '隆基股份'],
        ...
        while cache == False:
            try:
                ...
                self.stock_data['close']=self.stock_data['close'].astype('float')
                self.stock_data['pctChg']=self.stock_data['pctChg'].astype('float')
                date=list(set(list(self.stock_data['date'])))
                data={}
                for i in self.names:
                    data[i]=list(self.stock_data[self.stock_data['name']==i]['close'])
                line(data,date,"picture\\close")
                data={}
                for i in self.names:
                    data[i]=list(self.stock_data[self.stock_data['name']==i]['pctChg'])
                line(data,date,"picture\\pctChg")
                break
