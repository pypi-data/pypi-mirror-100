...
class StockData(object):
    ...
    def stock_pair(self):
        ...
        for i,j in df[['code','code_name','ipoDate']].iterrows():
            if j[2] >= self.start_date:
                self.sprint.red(f"{j[1]}'s ipo date is {j[2]},which is after {self.start_date}.\nPlease check your start date!")
                raise ValueError("The start date should be after the ipo date!")
            stocks[j[1]] = j[0]
    ...
    def stock_data(self, info="date,code,open,close,volume,amount,adjustflag,turn,pctChg"):
        ...
        Name = dict(zip(stock_pair.values(), stock_pair.keys())) #交换stock_pair键值对
        lists = []
        ...
        for i in tqdm(codes):
            ...
            lists.append(result)
        df = pd.concat(lists)
        df=df[['date','name','code','open','close','volume','amount','adjustflag','turn','pctChg']]
        if self.cache == True:
...
class HolidayStockData(object):
    ...
    def HolidayNearbyDate(self):
        ...
        if Date.empty == True:
            self.sprint.red(f'There are no festivals during {self.start_date}--{self.end_date}')
...
class StockAbility(object):
    ...
    def __init__(self, names=['贵州茅台', '隆基股份'],
        ...
        self.sprint.red('Please make sure that all the data has already existed!')
        self.names = names
        self.start_year = start_year
        self.end_year = end_year
        self.start_quater = start_quater
        self.end_quater = end_quater
        self.slog.log(f'time: {self.start_year},{self.start_quater} ~~ {self.end_year},{self.end_quater}', mode=1)
        self.slog.log(f'stocks: {self.names}', mode=1)
        Range = []
