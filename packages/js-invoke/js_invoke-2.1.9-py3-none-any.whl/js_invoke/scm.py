...
def _get_default_branch(ctx):
    # Return the default branch or main if not defined
    default_branch = 'main'
    ...
    try:
        default_branch = result.stdout.splitlines()[0]
...
def _bootstrap_repo(ctx):
    ...
    template_dir = '/home/vagrant/.git-template'
    ctx.run('mkdir {}'.format(template_dir))
    ctx.run('echo "ref: refs/heads/main" > {}/HEAD'.format(template_dir))
    ctx.run('git config --global init.templateDir {}'.format(template_dir))
