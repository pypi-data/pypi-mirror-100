...
class ModelTrainer(object):
    ...
    def fit(self, X, y, X_val=None, y_val=None):
        ...
        if 'save-evaluation-output' not in self.config:
            self.config['save-evaluation-output']=True

        if 'evaluate-performance' not in self.config:
            self.config['evaluate-performance']=False

        if self.config['evaluate-performance']==True and self.config['save-evaluation-output']==True:
            self.performance_data=pd.DataFrame(np.column_stack((X, y, self.pipeline.final_estimator.validatation_data)))
            self.performance_data.columns=['content', 'class', 'prediction', 'probability']
        return self.pipeline.final_estimator
    ...
    def persist(self, path, fixed_model_name=None):
        ...
        if self.performance_data is not None:
            self.performance_data.to_excel(os.path.join(path, 'validatation_data.xlsx'))
