...
def PartitionGrid(number_of_x_blocks=None,
    ...
    for block in g_zone_list:
        ...
        for node_id in block.node_id_list:
            if (link_id_temp < 50):
                if (g_node_list[node_id].activity_location_tab in ['residential', 'boundary', 'poi']):

                    connector_link = Link(link_id = block.id * 100000 + link_id_temp,
                                        from_node_id = node_id,
                                        to_node_id = centroid_node.id,
                                        link_type_name = 'connector with ' + g_node_list[node_id].activity_location_tab,
                                        link_type_id = -1)

                    connector_link.geometry = 'LINESTRING (' + \
                                            str(g_node_list[node_id].x_coord) + ' ' + \
                                            str(g_node_list[node_id].y_coord) + ' ' + ',' + \
