...
class QGraph:
    ...
    def _refresh(self):
        qgraph_json = self._fetch_qgraph_json()
        new_graphs = QGraph._from_graph_dict(qgraph_json)
    ...
    def fit(self, data, n_samples:Optional[int]=None, loss_function=_feyn.DEFAULT_LOSS, show:Optional[FittingShowType]="graph", threads:int=4, sample_weights=None, criterion:str=None) -> "QGraph":
        ...
        if self.fit_count>0:
            ...
            self._refresh()
        self.fit_count += 1
        self._graphs = self.trainer.fit(self._graphs, data, n_samples, loss_function, threads, sample_weights)

        if criterion=="bic":
            self._graphs.sort(key=lambda g: g.bic, reverse=False)
        elif criterion=="aic":
            self._graphs.sort(key=lambda g: g.aic, reverse=False)
        elif criterion is None:
            self._graphs.sort(key=lambda g: g.loss_value, reverse=False)
        else:
    ...
    def _update_feature_importance(self, feature_importance: Dict):
        importance_dict = dict(feature_importance)

        minval = min(importance_dict.values())
        maxval = max(importance_dict.values())
        # Normalize it

        g = feyn.Graph(len(importance_dict))
        for i, (name,val) in enumerate(importance_dict.items()):
            v = (val-minval) / (maxval-minval)
            g[i] = feyn.Interaction("in:linear(f)->i",(i,-1,-1), (0,0,0), name)
            g[i]._strength = v
        self.qlattice.update(g)
    ...
    def sort(self, data, loss_function=_feyn.DEFAULT_LOSS, threads:int=4, sample_weights=None, criterion:str=None):
        ...
        Returns:
            QGraph -- The QGraph itself
        """
        self._graphs = self.trainer.sort(self._graphs, data, loss_function, threads, sample_weights)
    ...
    @staticmethod
    def _from_graph_dict(graph_dict: dict) -> Set[Graph]:
        ...
        for out_id in out_ids:
            ...
            new_graphs.add(QGraph._build_graph(nodelist))

        return new_graphs

    @staticmethod
    def _build_graph(nodelist: List[Dict]):
        sz = len(nodelist)
        graph = Graph(sz)
