...
class SpikeLogger():
    ...
    def log_input(self, input_data:dict, user:str="SpikeDefaultLogger"):
        ...
        log = LogInput(log_type="logInput", user=user, input_raw=input_data)
        self.logger.log_struct(log.dict())

    def log_info(self, info:dict=None, user:str="SpikeDefaultLogger"):
        ...
        log = LogInfo(log_type="logInfo", user=user, project=self.project_name, date=log_date, info=info)
        self.logger.log_struct(log.dict())
    ...
    def log_prediction(self, 
        ...
        pred = Prediction(prediction=prediction, model_id=model_id, process_time=process_time, prediction_date=log_date, model_features=input_features, user=user)
        log_in = LogInput(log_type="logInput", user=user, input_raw=input_raw) 
        
        log = LogPrediction(log_type="logPrediction", predict=pred, input=log_in, user=user)
        self.logger.log_struct(log.dict())
        pass
    ...
    def log_prediction_error(self, 
        ...
        log_date = str(datetime.now())
        log_in = LogInput(log_type="logInput", user=user, input_raw=input_data,)
        log = LogPredictionError(log_type="logPredictionError", msg_error=msg_error, model_id=model_id, date=log_date, input=log_in, user=user)
        self.logger.log_struct(log.dict())