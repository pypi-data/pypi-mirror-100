...
class TestsSherlock(unittest.TestCase):
    ...
    def test_setup_detrend(self):
        sherlock = Sherlock(False, [SherlockTarget(object_info=None, smooth_enabled=False, high_rms_enabled=False,
    ...
    def test_setup_transit_adjust_params(self):
        sherlock = Sherlock(False, [SherlockTarget(object_info=None, max_runs=5, period_protect=5, period_min=1,
    ...
    def test_scoring_algorithm(self):
        sherlock = Sherlock(False, [SherlockTarget(object_info=None, best_signal_algorithm="basic")])
        self.assertTrue(isinstance(sherlock.sherlock_targets[0].signal_score_selectors["basic"], BasicSignalSelector))
        sherlock = Sherlock(False, [SherlockTarget(object_info=None, best_signal_algorithm="border-correct")])
        ...
        sherlock = Sherlock(False, [SherlockTarget(object_info=None, best_signal_algorithm="quorum")])
    ...
    def test_run(self):
        ...
        try:
            Sherlock(False, [SherlockTarget(MissionFfiIdObjectInfo("TIC 181804752", 'all'),
    ...
    def test_run_with_rms_mask(self):
        run_dir = "TIC181804752_FFI_all"
        try:
            Sherlock(False, [SherlockTarget(MissionFfiIdObjectInfo("TIC 181804752", 'all'), high_rms_enabled=True,
                                            max_runs=1)]).run()
    ...
    def test_run_with_autodetrend(self):
        run_dir = None
        try:
            Sherlock(False, [SherlockTarget(MissionObjectInfo("TIC 259377017", [5]), detrends_number=1,
    ...
    def test_run_epic_ffi(self):
        run_dir = None
        try:
            Sherlock(False, [SherlockTarget(MissionFfiIdObjectInfo("EPIC 249631677", 'all'),
    ...
    def test_run_with_transit_customs(self):
        run_dir = None
        try:
            sherlock = Sherlock(False, [SherlockTarget(MissionFfiIdObjectInfo("TIC 181804752", 'all'),
