...
class EPSON_invoice:
	...
	def print_order_invoice(self, order_info, printer_name="ARGOX_D2-250_PPLB", **kwargs):
		...
		sf.append({"string": "結帳單     %s"%(order_info.get_stayortogo_str()), "font": "big_font"})
		sf.append({"string": "單號: %s"%(str(order_info.serial_number)), "font": "big_font"})
		...
		total_price = 0
		for i in range(len(order_info.orders)):
			total_price += order_info.orders[i].price

			# handle the title string
			# title_list = order_info.orders[i].title.split(" ")
			# sf.append({"string": "%-2d %-20s %d        %d"%(i+1, title_list[0], order_info.orders[i].amount, order_info.orders[i].price), "font": "small_font"})
			self.append_order_item(sf, i+1, order_info.orders[i].title, order_info.orders[i].amount, order_info.orders[i].price)

			# handle the option string
			if order_info.orders[i].get_options_str() != "":
				option_str = sf.split_string(sf.small_font, "    %s"%(order_info.orders[i].get_options_str()))
		...
		if int(order_info["plastic_bag"]) > 0:
			total_price += int(order_info["plastic_bag"])
			self.append_order_item(sf, len(order_info.orders)+1, order_info.get_plastic_bag_str(), 1, int(order_info["plastic_bag"]))
		...
		sf.add_repeat_char(" ", "small_font")
		sf.append({"string": "折 扣 %37d"%(order_info["total_discount"]), "font": "small_font"})
		sf.add_repeat_char(" ", "small_font")

		sf.add_repeat_char("=", "small_font")
		sf.add_repeat_char(" ", "small_font")

		sf.append({"string": "總 計 %37d"%(total_price - order_info["total_discount"]), "font": "small_font"})
		
		self.img_draw_string(d, sf)

		# handle order qrcode
		qrcode_path = self.epson_tmpfile_path+"/order_qrcode.png"
		self.generate_qrcode_and_save(order_info.order_id, qrcode_path)
