...
class ModelValidator:
	...
	def _resolve_field_definition(self, fqn, field_name, dependency_values):
		...
		if definition.is_one_of():
			...
			try:
				definition_name = self.schema.dependency_resolver.resolve(fqn, field_name, dependency_values)
	...
	def _iterate(self, validated, fqn, values, callback, **rules_kw):
		...
		for field_name in self.dependency_order:
			...
			# Default Value
			###############
			field_definition = self._resolve_field_definition(child_fqn, field_name, result)
