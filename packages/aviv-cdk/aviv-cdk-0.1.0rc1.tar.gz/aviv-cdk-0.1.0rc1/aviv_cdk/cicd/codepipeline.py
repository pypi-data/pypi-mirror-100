...
class Pipeline(cp.Pipeline):
    ...
    def source(self, action_name: str, repository: typing.Union[str, sources.SourceRepositoryAttrs]=None, **source_args) -> cpa.BitBucketSourceAction:
        if not repository:
            info = sources.git_repository_info()
            repository = f"{info.get('url')}@{info.get('branch')}"
        if isinstance(repository, str):
            repository = sources.github_url_split(repository)
    ...
    def build_project(self, id: str, *, specfile: str='buildspec.yml', **project_args) -> cb.Project:
        ...
        return cb.Project(self, id, **project_args)

    def build(self, action_name: str, *,
            sources: list=[],
            project_args: dict={},
            **build_args) -> cpa.CodeBuildAction:
