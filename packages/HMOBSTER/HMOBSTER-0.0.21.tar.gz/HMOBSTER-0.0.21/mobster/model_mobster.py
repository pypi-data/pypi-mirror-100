...
@config_enumerate
def model(data, K=1, tail=1, purity=0.96, alpha_prior_concentration = 5, alpha_prior_rate = 0.1, number_of_trials_clonal_mean=500., number_of_trials_k=300.,
    ...
    for kr in pyro.plate("kr", len(karyos)):
        ...
        alpha_teo = pyro.sample('alpha_{}'.format(kr), dist.LogNormal(torch.log(alpha_prior * mut.theo_allele_list[karyos[kr]]), 1/alpha_prior_sd))
        ...
        # Here we split again the computational graph in case of tail or no tail
            if (tail == 1):
                ...
                alpha = pyro.sample("alpha_noise_{}".format(kr),
                                    dist.LogNormal(torch.log(alpha_teo), 1 / alpha_precision))
