...
class Paragraph:
    ...
    def add_content(self, content, type):
        ...
        if type==ContentType.Table:
            self.content.append({'type': type, 'content': content.to_pandas(),'top': content.top, 'bottom': content.bottom})
        else:
        ...
        if len(self.content) >=2 and self.content[-1]['type']== ContentType.Table and self.content[-2]['type']==ContentType.Table:
            if self.content[-1]['top']-self.content[-2]['bottom'] < 2*content.interline :
