...
class CommonModel(ABC):
    ...
    def fit(self, X_train, y_train, sample_weight=None):
        ...
        if (self.use_gpu == True) and (self.use_histograms == False):
            raise ValueError("GPU acceleration can only be enabled if use_histograms=True")

        self.check_gpu()

        if _mpi_enabled:
            raise ValueError("MPI (distributed) implementation of decision tree classifiers not yet supported.")
        ...
        if len(y_train) != num_ex:
            raise ValueError("Inconsistent dimensions: X_train.shape[0] must equal len(y_train)")
        ...
        if (isinstance(self.min_samples_leaf,(int))):
            if (self.min_samples_leaf > num_ex):
                raise ValueError("Parameter min_samples_leaf: invalid value. The value of min_samples_leaf is larger than the number of examples in the train dataset.")
            else:
    ...
    def predict_proba(self, X, n_jobs=0):
        ...
        # Generate predictions
        proba = self.c_predict(X.shape[0],
                               X.shape[1],
                               np.array([]),
                               np.array([]),
                               data,
        ...
        out = np.zeros(shape=(proba.shape[0], 2), dtype=proba.dtype)
        out[:,0] = 1 - proba
        out[:,1] = proba
