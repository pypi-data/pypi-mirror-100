...
class Simulation():
	...
	def getPlotQuantities(self, q_plot):
		"""
		...
		q_plot: array_like
			...
			:data:`~ulula.simulation.fields` dictionary.
		...
		nq = len(q_plot)
		q_array = np.zeros((nq, self.nx + 2 * self.nghost, self.ny + 2 * self.nghost), np.float)

		for iq in range(nq):
			q = q_plot[iq]
			if q == 'DN':
				q_array[iq] = self.V[DN]
			elif q == 'VX':
				q_array[iq] = self.V[VX]
			elif q == 'VY':
				q_array[iq] = self.V[VY]
			elif q == 'PR':
				q_array[iq] = self.V[PR]
			elif q == 'MX':
				q_array[iq] = self.U[MX]
			elif q == 'MY':
				q_array[iq] = self.U[MY]
			elif q == 'ET':
				q_array[iq] = self.U[ET]
			elif q == 'VT':
				q_array[iq] = np.sqrt(self.V[VX]**2 + self.V[VY]**2)
			else:
	...
	def plot1D(self, q_plot = ['DN', 'VX', 'VY', 'PR'], plot_type = 'line', 
		...
		nq_plot = len(q_plot)
		q_array = self.getPlotQuantities(q_plot)
		...
		if plot_type == 'line':
			...
			xmin = 0.0
			nx_half = self.nx // 2 + self.nghost
			ny_half = self.ny // 2 + self.nghost
			x_half = self.xmax * 0.5
			y_half = self.ymax * 0.5
			if self.nx >= self.ny:
				xmax = 0.5 * (self.ymax - self.ymin)
				n_cells = self.nx
			else:
				xmax = 0.5 * (self.xmax - self.xmin)
				n_cells = self.ny
			...
			cell_x, cell_y = self.xyGrid()
			cell_x = cell_x[(slc_x, slc_y)]
			cell_y = cell_y[(slc_x, slc_y)]
			circle_weight = np.zeros((n_r, n_cells, n_cells), np.float)
			for i in range(n_r):
				circle_weight[i] = utils.circleSquareOverlap(x_half, y_half, bin_edges[i + 1], cell_x, cell_y, self.dx)
		...
		if true_solution_func is not None:
			V_true = true_solution_func(self, x_plot, q_plot)
		...
		for i in range(nq_plot):
			...
			plt.plot(x_plot, V_line[i, :], color = 'darkblue', label = 'Solution, t=%.2f' % (self.t))
