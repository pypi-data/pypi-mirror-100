...
SH_LEAD_CODE_ARR = ['5','6','9'] 				# 沪市股票代码前导数字(5开头是沪市基金，6开头是沪市A股，9开头是沪市B股)
SZ_LEAD_CODE_ARR = ['0','1','2','3'] 			# 深市股票代码前导数字(1开头是深市基金，0和3开头是深市A股，2开头是深市B股)
...
SH60 = 'SH60' 			# 表示沪市 60 开头（A股）的股票类别名称
SH688 = 'SH688' 		# 表示沪市 688 开头（A股科创板）的股票类别名称
SH900 = 'SH900' 		# 表示沪市 900 开头（B股）的股票类别名称
SH5 = 'SH5' 			# 表示沪市 5 开头的证券（基金）类别名称

SZ00 = 'SZ00' 			# 表示深市 00 开头（A股）的股票类别名称
SZ300 = 'SZ300' 		# 表示深市 300 开头（A股创业板）的股票类别名称
SZ200 = 'SZ200' 		# 表示深市 200 开头（B股）的股票类别名称
SZ1 = 'SZ1' 			# 表示深市 1 开头的证券（基金）类别名称
...
# ---------------
# 沪市证券代码正则分类
SH60_RE = '^60' 		# 沪市 60 开头的代码（代表A股主板股票）
SH688_RE = '^688' 		# 沪市 688 开头的代码（代表A股科创板股票）
SH900_RE = '^900' 		# 沪市 9 开头的代码（代表B股股票）
...
SZ300_RE = '^300' 		# 深市 300 开头的代码（代表A股的创业板）
SZ200_RE = '^200' 		# 深市 200 开头的代码（代表B股）
SZ1_RE = '^1' 			# 深市 1 开头的代码（代表基金）
...
SH900_SYMBOL_ARR = [SH900,SH900_RE,'900',900] 				# 定义一个各种可表示沪市B股（900开头的股票）股票的识别标志
SH5_SYMBOL_ARR = [SH5,SH5_RE,'5',5] 						# 定义一个各种可表示沪市基金（5开头的证券）的识别标志
...
SZ200_SYMBOL_ARR = [SZ200,SZ200_RE,'200',200] 				# 定义一个各种可表示深市 B股（即200开头的股票）股票的识别标志
SZ1_SYMBOL_ARR = [SZ1,SZ1_RE,'1',1] 						# 定义一个各种可表示深市基金（1开头的证券）的识别标志
