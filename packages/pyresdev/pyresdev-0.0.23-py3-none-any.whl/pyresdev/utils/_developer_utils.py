...
def set_idemailstatus_target_column(df):
    ...
    if ('IDEmailStatus' in df.columns):
        ...
        status_col = df.pop( 'IDEmailStatus')
        df.insert(loc=0, value=status_col.astype('int32'),column= 'IDEmailStatus')
    else:
...
def set_bv_target_column(df):
    if('BV' in df.columns):
        ...
        df['BV']=df['BV'].astype('float32')
        bv_col = df.pop( 'BV')
        df.insert(loc=0, value=bv_col.astype('float32'),column= 'BV')
    else:
