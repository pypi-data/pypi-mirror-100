...
class GoogleNewsEngine(ACMEBrowser):
    ...
    def parse_news_contents(self, url: str) -> Dict:
        ...
        return {'title': self.google_driver.title,
                'url': self.google_driver.current_url,
                'content': self.google_driver.find_element_by_tag_name('body').text,
                'page_source': self.google_driver.page_source
                }