...
class RussiaToday(ACMEBrowser):
    ...
    def get_video_element(self, rt_driver: WebDriver, content_type: str) -> List:
        try:
            if content_type == 'image':
                return [x.get_attribute("content") for x in rt_driver.find_element_by_tag_name('head').find_elements_by_tag_name('meta') if "png" in str(x.get_attribute("content"))]

            if content_type == 'video':
                return [x.get_attribute("content") for x in rt_driver.find_element_by_tag_name('head').find_elements_by_tag_name('meta') if "mp4" in str(x.get_attribute("content"))]
        except:
            return []
