...
class FrameProcessor(object):
    ...
    def process_frame(self, frame, line_start, process_vars=True):
        ...
        snapshot_frame = SnapshotFrame(basename, func_name, lineno, filename, 0)
    ...
    def search_consumer(self, node):
        ...
        variable = self.process_variable_breadth_first(node_key_, node_value_)
        node.get_parent().add_child(variable)

        if not self.var_lookup[variable.id].flagged('redacted'):
    ...
    def process_variable_breadth_first(self, node_key_, node_value_):
        ...
        if hash_ in self.var_cache:
            ...
            return VariableId(var_id, node_key_)
        ...
        if self.client_config.black_list(node_key_):
            ...
            elif type_ is dict or type_.__name__ in LIST_LIKE_TYPES or \
                    type_.__name__ == 'listreverseiterator' or type_.__name__ == 'list_reverseiterator':
    ...
    def process_children_breadth_first(self, parent_id, value, curr_depth, var):
        ...
        if curr_depth + 1 >= self.max_depth:
            self.var_lookup[var.id].flag('depth')
            return []
