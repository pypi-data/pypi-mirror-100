...
from bavard_ml_common.types.agent import AgentExport, AgentConfig, AgentActionDefinition, Intent, Slot
...
def main():
    ...
    logger.info('Total training dialogues: {}', len(all_dialogues))
    ...
    logger.info('Total hotel dialogues: {}', len(conversations))

    hotel_data_agent = AgentExport(
        config=AgentConfig(
            actions=[AgentActionDefinition(name=action) for action in conversations.unique_labels()],
            intentExamples={intent: [] for intent in conversations.unique_intents()},
            intents=[Intent(name=intent) for intent in conversations.unique_intents()],
            slots=[Slot(name=slot) for slot in conversations.unique_slots()],
            tagTypes=list(conversations.unique_tag_types()),
            trainingConversations=list(conversations),
            uname="MultiWOZ-2.2"
        )
    )
