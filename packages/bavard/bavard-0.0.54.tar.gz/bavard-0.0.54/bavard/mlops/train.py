...
def train_and_evaluate(args):
    ...
    agent_config = AgentExport.parse_file(agent_filename).config
    ...
    logger.info('Publishing message to topic chatbot-service-training-jobs: {}', str(pubsub_msg))
    if not config.IS_TEST:
        PubSub(args.gcp_project_id).publish('chatbot-service-training-jobs', pubsub_msg)
    logger.info('Published JOB_COMPLETE message')
...
if __name__ == '__main__':
    ...
    try:
        ...
        logger.info('Publishing message to topic chatbot-service-training-jobs: {}', str(data))
        if not config.IS_TEST:
            PubSub(args.gcp_project_id).publish('chatbot-service-training-jobs', data)
        logger.info('Published ML_TRAINING_JOB_FAILED message')
        raise e
