...
class NN():
    ...
    Attributes:
        ...
        checkpoint (tf.train.Checkpoint): Groups trackable objects, saving and
            restoring them.
    """
    def __init__(self, model, one_hot_labels=False):
        ...
        for epoch in range(epochs):
            ...
            for examples in training_data:
                ...
                if self.one_hot_labels:
                    train_acc_batch_avg.update_state(
                        tf.argmax(prediction, axis=1), tf.argmax(labels,
                                                                 axis=1))
                    train_acc_epoch_avg.update_state(
                        tf.argmax(prediction, axis=1), tf.argmax(labels,
                                                                 axis=1))
                else:
                    train_acc_batch_avg.update_state(
            ...
            for examples in validation_data:
                ...
                if self.one_hot_labels:
                    val_acc_avg.update_state(tf.argmax(prediction, axis=1),
    ...
    def evaluate(self, dataset):
        ...
        for images, labels in dataset:
            ...
            if self.one_hot_labels:
                acc_epoch_avg.update_state(tf.argmax(prediction, axis=1),
                                           tf.argmax(labels, axis=1))
            else:
                acc_epoch_avg.update_state(tf.argmax(prediction, axis=1),
                                           labels)
        ...
        result = {
            "loss": round(float(loss_epoch_avg.result()), 4),
            "accuracy": round(float(acc_epoch_avg.result()), 4)
        }
