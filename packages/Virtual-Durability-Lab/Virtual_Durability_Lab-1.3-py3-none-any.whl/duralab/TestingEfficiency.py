...
class PlanEfficiency:
            ...
            for m in ["Normal", "Lognormal", "Exponential", "Weibull"]:
                ...
                if estimation_dict[m] != {}:
                    ...
                    for name, value in estimation_dict[m].items():
                        ...
                        value = value if m != "Exponential" else 1/value
                        
                        if i==0:
                            self.estimation_result_efficiency[m][name] = {"value" : [value], "mean" : [value], "std" : [value], "var" : [value] }
                                
                        else:
                            for indicator in ["value", "mean", "std", "var"]:
                                self.estimation_result_efficiency[m][name][indicator] += [value]
        
        for m in ["Normal", "Lognormal", "Exponential", "Weibull"]:
            for name in self.estimation_result_efficiency[m].keys():
                l = len(self.estimation_result_efficiency[m][name]["value"])+1
                self.estimation_result_efficiency[m][name]["mean"] = [ np.mean(self.estimation_result_efficiency[m][name]["value"][:i]) for i in range(1, l)] 
                self.estimation_result_efficiency[m][name]["std"] = [ np.std(self.estimation_result_efficiency[m][name]["value"][:i]) for i in range(1, l)]
                self.estimation_result_efficiency[m][name]["var"] = [ np.var(self.estimation_result_efficiency[m][name]["value"][:i]) for i in range(1, l)]
    ...
    def efficiency_visualization(self):
        ...
        title = {"Normal":{"r_value":"R", "s": "σ", "m": "μ"}, 
                 "Lognormal":{"r_value":"R", "sln":"σ", "mln": "μ"}, 
                 "Exponential":{"r_value":"R", "l":"1/λ"}, 
                 "Weibull":{"r_value":"R", "b":"β", "h": "η"}}
        ...
        for i, (law, estimation) in enumerate(self.estimation_result_efficiency.items()):
            ...
            if estimation != {}:
                ...
                for j, (name, value) in enumerate(estimation.items()):
                    ...
                    for k, indicator in enumerate(["value", "mean", "std", "var"]):
                        axs[j, i].plot(x, value[indicator], color=["black", "red", "green", "blue"][k], label="{} {}".format(title[law][name], indicator))
                    
                    axs[j, i].set_title("{} {}".format(law, title[law][name]))
                    axs[j, i].set_xlabel("Repetition number")
