...
from congas.utils import log_sum_exp, entropy_CNV
...
class LatentCategorical(Model):
    ...
    def model(self,i = 1,  *args, **kwargs):
        ...
        Temperature = self._params['Temperature'] / np.log2(i)
        ...
        if self._params["latent_type"] == "D":
            pyro.factor("lk", self._params['lambda'] * lk_rna + (1-self._params['lambda']) * lk_atac - torch.tensor(N * M / self._params['hidden_dim'], dtype = torch.float) * entropy_CNV(cc))
        else:
