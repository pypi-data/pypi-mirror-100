...
from congas.utils import log_sum_exp, entropy_CNV, entropy_per_segment
    ...
    def model(self,i = 1,  *args, **kwargs):
        ...
        if 'data_rna' in self._data:
            ...
            with pyro.plate('data_rna', N, batch1):
                ...
                if self._params["latent_type"] == "D":
                    ...
                    per_segment_ass_rna = torch.exp(lk_rna_aux - log_sum_exp(lk_rna_aux))
        ...
        if 'data_atac' in self._data:
            ...
            with pyro.plate('data_atac', M, batch2):
                ...
                if self._params["latent_type"] == "D":
                    ...
                    per_segment_ass_atac = torch.exp(lk_atac_aux - log_sum_exp(lk_atac_aux))
        ...
        if self._params["latent_type"] == "D":
            pyro.factor("lk", self._params['lambda'] * lk_rna + (1-self._params['lambda']) * lk_atac -
                        torch.tensor((self._params['lambda'] * N) * ((1 - self._params['lambda']) * M) / self._params['hidden_dim'], dtype = torch.float) * entropy_CNV(cc))
        else:
            entropy_per_segments = self._params['lambda'] * entropy_per_segment(per_segment_ass_rna) + (1-self._params['lambda']) * entropy_per_segments_atac
            lk_total = self._params['lambda'] * lk_rna + (1-self._params['lambda']) * lk_atac
            pyro.factor("lk", lk_total + entropy_per_segments)
