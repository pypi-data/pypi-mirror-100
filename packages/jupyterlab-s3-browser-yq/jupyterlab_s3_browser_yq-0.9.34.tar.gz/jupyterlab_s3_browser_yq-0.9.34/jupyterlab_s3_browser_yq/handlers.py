...
def test_s3_credentials(endpoint_url, client_id, client_secret, session_token):
    ...
    """
    app_log.info("testing s3 credentials")
    test = boto3.resource(
        "s3",
...
class AuthHandler(APIHandler):  # pylint: disable=abstract-method
    ...
    def get(self, path=""):
        ...
        if has_aws_s3_role_access():
            authenticated = True
            app_log.info("S3 Role Access")

        if not authenticated:

            try:
                config = self.config
                app_log.info(str(f"COnfig in GET before IF {config.client_id}"))
                if config.endpoint_url and config.client_id and config.client_secret:
                    app_log.info(str(f"Config inside IF {config.session_token}"))
                    test_s3_credentials(
                        config.endpoint_url,
                        config.client_id,
    ...
    def post(self, path=""):
        ...
        try:
            ...
            app_log.info(str(f'POST Request: {req}, session token is {session_token}'))
...
class S3Handler(APIHandler):
    ...
    def get(self, path=""):
        ...
        """
        app_log.info("GET: {}".format(path))

        #  boto3.set_stream_logger("boto3.resources", app_log.info)
        #  boto3.set_stream_logger("botocore", app_log.info)
        try:
            if not self.s3:
