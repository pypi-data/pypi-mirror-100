...
def group_small(feat, low_counts, label="other"):
    """
    Group levels with fewer than min_samples.

    :param feat: DataSeries, single feature to be processed
    :param low_counts: list of values to be grouped
    :param label: string, new label for all rejected levels
    :return: feat: DataSeries, processed feature
    """
    logger.info(f"Applying groupings for small levels = {low_counts}...")
    idxs_to_replace = np.where(np.isin(feat, low_counts))[0]
    feat.iloc[idxs_to_replace] = label
...
def apply_categorical_bounds(feat, levels, method):
    ...
    if method == CommonActions.DROP_SAMPLE:
        print("No missing values should exist after drop samples has been applied")
