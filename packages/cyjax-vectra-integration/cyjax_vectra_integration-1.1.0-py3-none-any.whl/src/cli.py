...
def setup_vectra_module():
    ...
    vectra_daily_threat_feed_id = input('Please provide the Vectra Threat feed ID for daily indicators:')
    vectra_weekly_threat_feed_id = input('Please provide the Vectra Threat feed ID for weekly indicators:')

    try:
        configuration.set_config(cyjax_api_key, vectra_fqdn, vectra_api_key, vectra_daily_threat_feed_id,
                                 vectra_weekly_threat_feed_id)
