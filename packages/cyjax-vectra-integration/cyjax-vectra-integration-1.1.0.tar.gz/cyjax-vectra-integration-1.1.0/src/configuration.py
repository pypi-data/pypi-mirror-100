...
VECTRA_DAILY_THREAT_FEED_ID = 'vectra_daily_threat_feed_id'
VECTRA_WEEKLY_THREAT_FEED_ID = 'vectra_weekly_threat_feed_id'
...
class Configuration:
    ...
    def get_daily_stix_file_path(self) -> str:
        """
        Returns the daily STIX file path.
        :return: The daily STIX file path.
        """
        return self.get_config_path() + '/stix-' + datetime.now().strftime("%Y%m%d") + '.xml'

    def get_weekly_stix_file_path(self) -> str:
        """
        Returns the weekly STIX file path.
        :return: The weekly STIX file path.
        """
        return self.get_config_path() + '/stix-' + datetime.now().strftime("%Y%W") + '.xml'
    ...
    def get_vectra_threat_feed_ids(self):
        """
        Returns the Vectra threat feed IDs.
        :return: The Vectra threat feed IDs.
        """
        return self._get_config(VECTRA_DAILY_THREAT_FEED_ID), self._get_config(VECTRA_WEEKLY_THREAT_FEED_ID)
    ...
    def set_config(self, cyjax_api_key: str, vectra_fqdn: str, vectra_api_key: str, vectra_daily_threat_feed_id: str,
                   vectra_weekly_threat_feed_id: str):
        """
        ...
        :param vectra_daily_threat_feed_id: The Vectra threat daily feed ID.
        :param vectra_weekly_threat_feed_id: The Vectra threat weekly feed ID.
        """
        self.config[CYJAX_API_KEY] = cyjax_api_key
        self.config[VECTRA_FQDN] = vectra_fqdn
        self.config[VECTRA_API_KEY] = vectra_api_key
        self.config[VECTRA_DAILY_THREAT_FEED_ID] = vectra_daily_threat_feed_id
        self.config[VECTRA_WEEKLY_THREAT_FEED_ID] = vectra_weekly_threat_feed_id
        ...
        try:
            vectra_client = VectraClient(self.config[VECTRA_FQDN], self.config[VECTRA_API_KEY],
                                         self.get_vectra_threat_feed_ids(), False)
    ...
    def validate(self):
        ...
        if VECTRA_DAILY_THREAT_FEED_ID not in self.config or not self.config[VECTRA_DAILY_THREAT_FEED_ID]:
            raise InvalidConfigurationException('The Vectra daily threat feed ID cannot be empty.')

        if VECTRA_WEEKLY_THREAT_FEED_ID not in self.config or not self.config[VECTRA_WEEKLY_THREAT_FEED_ID]:
            raise InvalidConfigurationException('The Vectra weekly threat feed ID cannot be empty.')
