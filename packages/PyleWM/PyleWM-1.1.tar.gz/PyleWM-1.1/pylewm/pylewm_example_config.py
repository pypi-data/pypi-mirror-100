...
# Main modifier key(s) that all the hotkeys are behind
# By default all hotkeys are CTRL+ALT+{}, but you may want to choose
# something a bit more ergonomic.
# It is *possible* to use the windows key ('win',) but you will not
# be able to override any of the default windows shortcuts, so you
# will have to work around them.
MOD = ('ctrl', 'alt')

# Apply the default config from PyleWM. You can remove this to
# prevent any of the default hotkeys from being bound.
import pylewm.pylewm_default_config
pylewm.pylewm_default_config.apply(MOD)

# The following hotkeys are the defaults applied above.
# Uncomment them to override them here.
...
# EXAMPLE: Hotkeys from the defaults can also be removed:
    #(*MOD, 'a')              : None,
