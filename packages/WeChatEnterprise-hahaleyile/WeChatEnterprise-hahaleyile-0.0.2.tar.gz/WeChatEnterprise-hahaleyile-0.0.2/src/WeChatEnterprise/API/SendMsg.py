...
class WeChatEnterprise:
    ...
    def __init__(self, corp_id: str, corp_secret: str, agent_id: str):
        self.r = redis.Redis(host=os.getenv('REDIS_HOST'), port=int(os.getenv('REDIS_PORT', default='6379')))
    ...
    def __get_local_access_token(self) -> bytes:
        return self.r.get(self.__DateBaseName + '_access_token')

    def __get_update_access_token(self, corp_id: str, corp_secret: str) -> dict:
        response: dict = requests.get(url="https://qyapi.weixin.qq.com/cgi-bin/gettoken",
