...
def _load_standard_library():
    path = os.path.abspath(os.dirname(__file__),
                           'data_files/standard_library.json')
    return set(load(path))


SENTINEL = object()
STANDARD_LIBRARY = _load_standard_library()


def in_standard_library(package_name):
    """Check if a package is in Python's standard library. Helpful when
    determining what pypi packages need to be installed for a project.
    """
