...
class Socket(object):
    ...
    def read(self, timeout_override: int=0) -> tuple[bool, bytes]:
        """
        Attempts to read data from the socket object. Returns 'bytes' object of response.
...
class ServerBase:
    ...
    def __del__(self):
        """
        Ensure the listening socket is properly closed, and the listening thread exits gracefully.
        """
        if self.running is True:
            self.stop()


def echo_callback(addr, sock: Socket):
    data = sock.read()[1]
    sock.write(data)
    sock.disconnect()
...
def server_test():
    ...
    conn.write(b"If you see this message, that means the sockets library is working!")
