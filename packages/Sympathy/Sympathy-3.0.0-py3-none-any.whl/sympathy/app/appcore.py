...
from sympathy.utils import uuid_generator, log, prim
...
class AppCore(QtCore.QObject):
            ...
            if node:
                self.display_message(
                    messages.StreamMessage(
                        level=level, id=(
                            taskid, ident, level), brief=text, node=node))

        elif msg.type == msgmod.CredentialRequestMessage:
            resource, mode = msg.data
            ok, secrets = credentials.instance().request_resource(
                resource, mode)
            if not ok:
                secrets = None
            self.message_reply(taskid, msg.reply(secrets))
        elif msg.type == msgmod.CredentialEditMessage:
            resource, mode, secrets = msg.data
            ok, secrets = credentials.instance().edit_resource(
    ...
    def execute_node_parameter_view_done(self, node_, result):
        ...
        if node_ is not None:
            ...
            if not old_params.equal_to(new_params):
                ...
                new_version = node_.auto_migrated_version()
                if new_version != node_.version:
                    migr_logger.info(
                        "Auto migrating node %s: %s -> %s",
                        node_.identifier, node_.version, new_version)
                    cmd = user_commands.MigrateNode(node_, new_version)
                ...
                if node_.has_overrides():
                    ...
                    new_params.set_version(new_version)
    ...
    def handle_subflow_parameter_view_done(self, full_uuid, flow_info_dict):
        ...
        for node, param_model, override_flow in changed_item_list:
            # Auto migrate node if needed
            new_version = node.auto_migrated_version()
            if new_version != node.version:
                migr_logger.info(
                    "Auto migrating node %s: %s -> %s",
                    node.identifier, node.version, new_version)
                cmd = user_commands.MigrateNode(node, new_version)
