...
class Flow(NodeStatusInterface, NodeInterface):
    ...
    def _get_overrides_migration_chain(self, tree_uuid):
        overrides_model = library.OverridesModel.from_dict(
            self.override_parameters[tree_uuid])
        ...
        if node is None or subflow is None:
            ...
            return None
        if not self._flow.app_core.is_node_in_library(node._identifier):
            return None
    ...
    def subflows_are_clean(self):
        ...
        dirty_subflows = set(dirty_flows) - (set(self.parent_flows()) - {self})
