...
def _update_calc_list(calc_list):
    ...
    # Build a regex which tries to match all backward references to previously
    # calculated columns.
    names_regex = r'|'.join(re.escape(name) for name in calcs.keys())
    res_regex = r'\${(' + names_regex + r')}'

    new_calc_list = []
    for name, calc in calcs.items():
        calc = re.sub(r'\btable\b', 'arg', calc)
        calc = re.sub(res_regex, r"res['\1']", calc)
