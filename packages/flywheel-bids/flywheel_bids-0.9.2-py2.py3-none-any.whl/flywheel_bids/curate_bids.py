...
def curate_bids_tree(fw, project, reset=False, template_type=None, template_file=None):
    ...
    if template_type:
        ...
        if template_type == "Default":
            logger.info("Using project curation template: Flywheel Default")
            template = templates.DEFAULT_TEMPLATE
        elif template_type == "ReproIn":
            logger.info("Using project curation template: Flywheel ReproIn")
            ...
            if template_filename:
                ...
                logger.info(f"Template downloaded from project: {template_filename}")
            ...
            logger.info("Unable to determine template file. Exiting.")
            # TODO: Create custom error for exit
            os.sys.exit(1)
...
def main_with_args(api_key, session_id, reset, session_only, template_type):
...
def main():
    ...
    parser.add_argument(
        ...
        action="store",
        default=None,
        help="Template file to use. Supercedes the --template-type flag.",
    )
