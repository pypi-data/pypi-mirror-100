            ...
            if config_hash not in cls.INSTANCE:
                ...
                if use_slave:
                    for index, one_slave in enumerate(slave_config):
                        ...
                        if name in slave_session_class_dict:
                            slave_session_class_dict[name] = [slave_session_class_dict[name]]
                            cur_engine = cls.get_engine(one_slave, charset, e_type)
                            slave_session_class_dict[name].append(cls.get_session_class(cur_engine))
                        else:
                            cur_engine = cls.get_engine(one_slave, charset, e_type)
    ...
    def sql_session(self, session_class_name=None, random_slave=False):
        ...
        if random_slave:
            if len(self.slave_session_class_dict) == 0:
                ...
                key = random.choice(list(self.slave_session_class_dict.keys()))
                key = session_class_name if session_class_name in self.slave_session_class_dict else key
                session_class = self.slave_session_class_dict[key] if type(
                    self.slave_session_class_dict[key]) != list else random.choice(self.slave_session_class_dict[key])
