...
class User:
    ...
    def get_user_maps(self, resultsPerPage, page):
        ...
        api = intersection.url.map_user(self.objectId, resultsPerPage, page)
...
def get_user(id):
    ...
    api = intersection.url.user_info(id)
...
def search_for_users(name):
    ...
    api = intersection.url.user_search(name)
    
    for i in range(len(api)):

        temp = User(
            int(api[i]['objectId']), 
            int(intersection.url.user_info(api[i]['objectId'])['gameVersion']),
            int(intersection.url.user_info(api[i]['objectId'])['lastLogin']),
            int(intersection.url.user_info(api[i]['objectId'])['maps']),
            intersection.url.user_info(api[i]['objectId'])['name'],
            int(intersection.url.user_info(api[i]['objectId'])['followers'])
        )
