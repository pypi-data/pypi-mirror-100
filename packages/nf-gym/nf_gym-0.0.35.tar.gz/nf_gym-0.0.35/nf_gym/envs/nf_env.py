...
class NFEnv(gym.Env):
    ...
    def __init__(self,show_gui=False,fall_rew=-1000.0,motion_rew=0,time_rew=0,height_rew=1.0,rand_force=None):
        self.step_counter = 0
        if show_gui:
            p.connect(p.GUI)
        else:
    ...
    def reset(self):
        ...
        try:
            self.bot_id = p.loadURDF("/Users/dj/stuff/bot/urdf/bot9.urdf", cubeStartPos, cubeStartOrientation)
        except:
            self.bot_id = p.loadURDF("/content/drive/My Drive/bot/bot9.urdf", cubeStartPos, cubeStartOrientation)
