...
from harp_gate_client.notifications_collectors.icinga_collector import IcingaCollector
from harp_gate_client.notifications_collectors.zabbix_collector import ZabbixCollector
import uuid
    ...
    def main(self):
        ...
        if self.monitoring_system == 'zabbix':
            for ms_source in settings.ZABBIX_SYSTEMS:
                if self.system_healthy(ms_source):
                    logger.info(
                        msg=f"Start scraping triggers from ms_source={ms_source}",
                        extra={'tags': {'event_id': str(self.event_id)}}
                    )
                    ...
                    logger.error(
                        msg=f"Monitoring system is unreachable: ms_source: {ms_source}",
                        extra={'tags': {'event_id': str(self.event_id)}}
                    )
        ...
        if self.monitoring_system == 'icinga':
            for ms_source in settings.ICINGA_SYSTEMS:
                logger.info(
                    msg=f"Start scraping alerts from ms_source={ms_source}",
                    extra={'tags': {'event_id': str(self.event_id)}}
                )
