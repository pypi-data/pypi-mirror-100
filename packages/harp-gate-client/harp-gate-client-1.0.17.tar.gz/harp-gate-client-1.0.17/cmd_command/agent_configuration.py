...
def is_root(ctx, param, value):
    if os.geteuid() == 0:
        ...
        raise click.BadParameter(f'You should run command from root user')
...
def update_config_file(new_config):
    ...
    if unique_integration in data:
        return f'Integration already exist in configuration - {unique_integration}'
    else:
...
@click.command()
@click.option('--yes', is_flag=True, callback=is_root, expose_value=True, prompt='Are you running command from root?')
