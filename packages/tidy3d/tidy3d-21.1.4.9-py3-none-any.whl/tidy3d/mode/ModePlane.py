...
class ModePlane(object):
    ...
    def _set_eps(self, sim, freqs, yee=True):
        """ Set the mesh of the ModePlane based on a global simulation mesh, 
        and the indexing of that mesh w.r.t. the simulation grid.
        ``yee`` determines if the permittivity at the Yee grid locations will
        also be set. This is currently not needed for plane-wave sources, which
        only check if the permittivity at the grid centers is uniform.
        """
