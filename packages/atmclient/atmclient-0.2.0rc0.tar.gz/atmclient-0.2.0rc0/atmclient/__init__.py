...
class ClientArgs(BaseSettings):
    gateway_host: str = "http://atm-api-gateway.atm.svc.cluster.local:8080" # https://atm.lxper.ai
    gateway_route_discovery_path: str = "/{endpoint_id}"
    gateway_route_generate_path: str = "/{endpoint_id}/generate"
    gateway_route_list_path: str = "/actuator/gateway/routes"
    
    auth_host: str = "https://auth.lxper.ai/auth/"
    ...
    @property
    def gateway_route_discovery_url(self):
        return urljoin(base=self.gateway_host, url=self.gateway_route_discovery_path)
    
    @property
    def gateway_route_generate_url(self):
        return urljoin(base=self.gateway_host, url=self.gateway_route_generate_path)
    
    @property
    def gateway_route_list_url(self):
        return urljoin(base=self.gateway_host, url=self.gateway_route_list_path)
...
class Client:
    ...
    Keyword Arguments:
        ...
        gateway_host (str): defaults to {default_gateway_host}
        
        gateway_route_discovery_path (str): defaults to {default_gateway_route_discovery_path}

        gateway_route_generate_path (str): defaults to {default_gateway_route_generate_path}

        gateway_route_list_path (str): defaults to {default_gateway_route_list_path}
        
        auth_host (str): defaults to {default_auth_host}
        
        auth_method (str): defaults to {default_auth_method}
        
        auth_realm (str): defaults to {default_auth_realm}
        
        client_id (str): defaults to {default_client_id}
        
        client_secret (Optional[str]): defaults to None
        
        username (Optional[str]): defaults to None
        
        password (Optional[str]): defaults to None
    ...
    Note:
        The priorty of settings
            Highest <<< Overrided via Initialization fed with **kwargs <<< Overrided via Environment variables (prefixed with 'AC_' ) <<< Default values <<< Lowest
    ...
    def _request(self, mode, endpoint_id, payload=None):
        ...
        try:
            ...
            # Everything good, auth failure not happened
            self._auth_not_failed()
            return data
        except (json.JSONDecodeError, requests.exceptions.HTTPError) as exc:
            # It says auth required
            if resp.status_code == 401:
                # Auth attempt increases by 1, and it raises error if too much auth attempt happens
                # If the auth attempt is not that high, tries to auth
                self._auth()
                return self._request(mode, endpoint_id, payload=payload)
            
            # Though something is bad, auth failure not happened.
            self._auth_not_failed()
    ...
    def _auth(self):
        ...
        if self._auth_failed >= 3:
            raise ATMClientError(f"[-] Unabled to authenticate user '{self._args.username}', client_id '{self._args.client_id}', "
                                 f"method: '{self._args.auth_method}', host: '{self._args.auth_host}'. "
                                )
        
        if self._args.auth_method == 'keycloak':
            try:
                self._auth_keycloak()
            ...
            arg_name = "auth_method"
            arg_value = self._args.auth_method
            
            raise ATMClientError(f"[-] Unexpected value, {arg_value!r} is not allowed for argument '{arg_name}'")
            

    def _auth_keycloak(self):
        # try to run keycloak auth
        from keycloak import KeycloakOpenID

        # Configure client
        keycloak_openid = KeycloakOpenID(
            server_url=self._args.auth_host,
            client_id=self._args.client_id,
            client_secret_key=self._args.client_secret,
            realm_name=self._args.auth_realm,
        )

        token = keycloak_openid.token(username=self._args.username, password=self._args.password)
        
        headers = {'Authorization': f'Bearer {token["access_token"]}'}
        self._sess.headers = headers

        
    def _auth_not_failed(self):
        # clean up the auth fail counter
        self._auth_failed = 0
