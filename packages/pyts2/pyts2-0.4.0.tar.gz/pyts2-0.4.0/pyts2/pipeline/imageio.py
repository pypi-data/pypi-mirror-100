    ...
    def process_file(self, file):
        ...
        try:
            if format in ("cr2", "nef", "rw2"):
                ...
                # use imageio to infer file format
                if file.format in (None, "", "."):

                    r = imageio.get_reader(file.content)
                    f = list(filter(lambda x: x.lstrip(".").lower() in self.default_options.keys(),
                                    r.format.extensions))
                    if len(f) > 0:
                        file.format = f[0].lstrip(".").lower()
