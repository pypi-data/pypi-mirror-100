# coding = utf-8
import psycopg2, re, Request_Receiver, os
def inst():
    ...
    while num <= Need_dict['num']:
        ...
        if Need_dict.get('kw') is None:
            if re.findall('\\.' + Need_dict['mode'], temp_data[0]) and not os.path.isfile('./Firmwares/' + re.sub("',\\)", '', re.split('/', temp_data[0])[-1])):
                data.append(temp_data)
                num += 1
        else:
            if re.findall(Need_dict['kw'], temp_data[0], re.I) and re.findall('\\.' + Need_dict['mode'], temp_data[0]) and not os.path.isfile('./Firmwares/' + re.sub("',\\)", '', re.split('/', temp_data[0])[-1])):
                data.append(temp_data)
                num += 1
    return data