...
class ExpaiExplanation:
    ...
    def plot(self, key: str = None):
        ...
        """
        assert key is not None, "You must specify a key to be plotted. You can use .get_plot_keys() to get all available keys."
        assert key in self.visualizations.keys(), "Your key doesn't exist in this explanation. You can use .get_plot_keys() to get all available keys."
    ...
    def plot_all(self):
        ...
        if isinstance(self.visualizations, dict):
            html = figures_to_html(self.visualizations.values())
    ...
    def store_explanation(self,
        ...
        data['explanation_json'] = {k: v for k, v in self.visualizations.items() if k in keys}
