...
from dofast.config import FOXMAIL_USERNAME
...
class PapaPhone:
    ...
    def entry(self, retry: int = 3) -> None:
        ...
        if retry <= 0:
            YahooMail().send(FOXMAIL_USERNAME, '手机余量查询失败', '已经连续重试 3 次，全部失败。')
            return
        ...
        if flow == -1:
            ...
            logger.info('\n'.join((FOXMAIL_USERNAME, subject, message)))
            YahooMail().send(FOXMAIL_USERNAME, subject, message)
