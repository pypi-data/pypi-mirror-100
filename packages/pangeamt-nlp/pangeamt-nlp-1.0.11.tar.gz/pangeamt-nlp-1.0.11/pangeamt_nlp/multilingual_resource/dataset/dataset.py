...
from pangeamt_nlp.multilingual_resource.aftxt.aftxt import AfTxt
...
class Dataset:
    ...
    def _explore(self):
        ...
        # aftxts
        aftxts = glob.glob(self._dir + '/*.txt')
        for aftxt in aftxts:
            self._resources.append(AfTxt(aftxt))

        files = glob.glob(self._dir + '/*')
        potential_bilinguals = []
        for f in files:
            if not f.endswith('.af') and not f.endswith('.tmx') and not f.endswith('.txt'): #TODO .af could  correspond to Afrikaans language
    ...
    def read(self, reader: DatasetReader, with_resource_index=False):
        ...
        for i, resource in enumerate(self._resources):
            try:
                ...
                elif resource.multilingual_resource_type == MultilingualResourceBase.TYPE_AFTXT:
                    current_reader = reader.aftxt_reader
