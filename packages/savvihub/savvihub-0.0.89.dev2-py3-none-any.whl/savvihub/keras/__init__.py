...
class SavviHubCallback(keras.callbacks.Callback):
    def __init__(self, data_type=None, validation_data=None, num_images=None, labels=None):
        super().__init__()
    ...
    def _inference(self):
        ...
        print(f'results len: {len(results)}, type: {type(results)}, val: {results}')
        print(f'predicts len: {len(predicts)}, type: {type(predicts)}, val: {predicts}')

        captions = []
        for predict, truth in zip(predicts, y_val_random):
            captions.append(f'Pred: {predict} Truth: {truth}')

        print(f'x_val_random len:{len(x_val_random)}, type: {type(x_val_random)}, type2: {type(x_val_random[0])}, val: {x_val_random}')
        print(f'caption len:{len(captions)}, type: {type(captions)}, type2: {type(captions[0])}, val: {captions}')
