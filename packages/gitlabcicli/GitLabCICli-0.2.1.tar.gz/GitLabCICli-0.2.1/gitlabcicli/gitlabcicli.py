...
def parse_args():
    ...
    parser_run_local = subparsers.add_parser(
        "run-local",
        aliases=COMMAND_ALIASES_MAP["run-local"],
...
class GitLabCiCli(object):
    ...
    def raw(self):
        ...
        for job in jobs:
            ...
            if not traces:
                error(
                    f"There is no job log yet for job #{job_id} for project '{self.args['project']}'."
                )

                continue
            elif len(traces) > 1:
                error(
                    f"There are too many job logs for job #{job_id} for project '{self.args['project']}'."
                )
...
def main():
    ...
    if getattr(args, "ask_for_token", None):
        debug("Try to get token from user input...", 2)
        token = getpass(f"Please enter the GitLab API Token for {args.server_url}: ")
    else:
        debug("Try to get token for server from config...", 2)
        token = get_token_from_keyring(args.server_url)
    ...
    debug(f"Using token {token}", 5)

    cli = GitLabCiCli(token=token, **args.__dict__)
    ...
    if token:
        keyring.set_password(KEYRING_SERVICE_NAME, args.server_url, token)
