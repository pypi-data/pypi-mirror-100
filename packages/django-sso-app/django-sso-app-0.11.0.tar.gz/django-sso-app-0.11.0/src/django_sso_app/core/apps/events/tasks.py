...
def _create_users_by_events(logger):
    ...
    for event_listener in instantiated_event_listeners:
        ...
        from_timestamp = from_date.replace(tzinfo=timezone.utc).timestamp()
...
def create_users_by_events(self, **kwargs):
    _logger = kwargs.get('logger', get_task_logger('{}.{}'.format(MODULE_NAME, 'create_users_by_events')))

    return _create_users_by_events(_logger)
