...
class NormalDistribution(Distribution):
    ...
    def __init__(self, mean_polynomial_coefficient: list, std_polynomial_coefficient: list, std_minus_mean: bool=False):
        ...
        self.std_minus_mean = std_minus_mean

    def sample(self, x: float = 1):
        mean = self.mean.calculate(x)
        return np.random.normal(mean, self.std.calculate(x) * (mean if self.std_minus_mean else 1))
