...
class AtlantisClient:
    ...
    :param bool skip_openid_configuration: Pula configuração do openid (Usado para ambiente de testes)
    """
    url = None
    client_id = None
    client_secret = None
    ...
    def __init__(self, url, client_id, client_secret=None, skip_openid_configuration=False):
        self.url = url.strip('/')
        self.client_id = client_id
        self.client_secret = client_secret
        self._jwks = None
        self._jwks_updated_at = 0
        self._jwks_cache_seconds = 600

        if skip_openid_configuration:
            return
    ...
    def get_jwks(self):
        ...
        :rtype: dict
        """
        if self._jwks is not None and time.time() - self._jwks_updated_at < self._jwks_cache_seconds:
            return self._jwks
        ...
        try:
            ...
            self._jwks_updated_at = time.time()
            return self._jwks
        except HTTPError as error:
            raise AtlantisClientHTTPError(error) from error
