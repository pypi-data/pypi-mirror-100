...
def tracer_config(__init__, app, args, kwargs):
    ...
    tracing._trace_all = app.settings.get("opentracing_trace_all", DEFAULT_TRACE_ALL)
    tracing._trace_client = app.settings.get(
        "opentracing_trace_client", DEFAULT_TRACE_CLIENT
    )
    tracing._start_span_cb = app.settings.get("opentracing_start_span_cb", None)
    tracing._trace_response_headers_enabled = app.settings.get(
        "signalfx_trace_response_headers", True
    )

    httpclient._set_tracing_enabled(tracing._trace_client)
    if tracing._trace_client:
        httpclient._set_tracing_info(tracing._tracer_obj, tracing._start_span_cb)
