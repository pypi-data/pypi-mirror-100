...
class UtilsDepartment(Department):
    
    files = flow.Child(FileSystemMap).ui(hidden=True)
    ...
    def compute_child_value(self, child_value):
        if child_value is self.path:
            ...
            settings = get_contextual_dict(self, "settings")
            self._file_prefix.set("lib_{asset_type}_utils_{dept}_".format(**settings))
        else:
            super(UtilsDepartment, self).compute_child_value(child_value)


class Template(Department):

    _parent_dept = flow.Parent(2)
    ...
    def compute_child_value(self, child_value):
        if child_value is self.path:
            ...
            settings = get_contextual_dict(self, "settings")
            file_prefix = self._parent_dept._file_prefix.get()
            self._file_prefix.set(file_prefix + "templates_" + self.name() + "_")
        else:
            super(Template, self).compute_child_value(child_value)


class Templates(flow.Map):

    create_template = flow.Child(baseflow.maputils.SimpleCreateAction)
...
class SkinningUtils(UtilsDepartment):
    
    _short_name = flow.Param("skinning")
    templates = flow.Child(Templates)


class RigModuleUtils(UtilsDepartment):
