...
def get_version_directory_path():
    return os.path.dirname(os.path.realpath(__file__)) + "\\..\\versions\\"
...
def get_version_dependant_structure_file(game_version: str, scenario_version: str, name: str) -> dict:
    try:
        ...
        with open(f"{vdir}\\{game_version}\\v{scenario_version}\\{name}.json", 'r') as structure_file:
...
def get_structure(game_version, scenario_version) -> dict:
    try:
        vdir = get_version_directory_path()
        with open(f"{vdir}\\{game_version}\\v{scenario_version}\\structure.json", 'r') as structure_file:
