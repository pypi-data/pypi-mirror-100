...
class Template():
    ...
    def add_subtemplate(self, subtemplate: dict = {}):
        ...
        for sid, service in services.items():
            d[_ID_TASK][sid] = parse_template(service, SERVICE_STRUCT, SERVICE_INTERNALS, sid)
            variable_key = d[_ID_TASK][sid].get('__variable__')
            variables = merge(variables.get(variable_key, {}), d[_ID_TASK][sid].get(_ID_VAR))
            d[_ID_TASK][sid][_ID_VAR] = variables
    ...
    def build(self, match: bool = True):
        ...
        for sid, service in services.items():
            ...
            variables = merge(variables.get(key, {}), service.get(_ID_VAR))
            service[_ID_VAR] = variables
