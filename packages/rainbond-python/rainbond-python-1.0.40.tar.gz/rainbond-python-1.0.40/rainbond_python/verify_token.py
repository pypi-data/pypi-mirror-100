...
def set_token_session(app, verify, per_defaults: list, whitelist: list = []):
    ...
    def base_auth():
        ...
        if user['user_name'] in admin_user:
            pass
        elif verify.group_list:
            meta_auth.update(group_list={'$elemMatch': {'$in': verify.group_list}})
        else:
            handle_abnormal(message='用户无任何群组权限，无法操作数据', status=403)

        # 根据模块保存对应权限（增删改查权限，群组权限，个人权限）
        for per in per_defaults:
            # 深拷贝处理分组权限条件数据
            auth_filter = copy.deepcopy(meta_auth)
            # 获取模块对应页面权限
            session[per['session_key']] = verify.permission_verify(
                per['center_name'], per['permission_name'])
            # 判断页面是否为个人权限，个人权限则添加创建人搜索条件
            if not session[per['session_key']]['is_all_data']:
                auth_filter.update(creator=user['user_name'])
            # 根据用户的分组权限，个人权限设置数据赛选条件，写入session
            session[per['session_key']].update(auth_filter=auth_filter)
...
class VerifyToken:
    ...
    def permission_verify(self, center_name, permission_name) -> dict:
        ...
        self.group_list = user['group_name']  # 用户包含的所有组别名称
        # 合并多角色权限
        permission_list = []
        is_all_data_list = []
        # 遍历角色权限数据
