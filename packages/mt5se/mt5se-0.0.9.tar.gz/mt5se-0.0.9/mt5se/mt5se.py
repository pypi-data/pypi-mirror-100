...
# Current value in portfolio of a given symbol, or total value of alls symbol, if None is given
def get_position_value(symbolId=None):
    ...
    if len(pos)>0:
        ...
        return 0.0
#se.get_position( symbol_id) # return the current position in a given asset (symbol_id)
#Examples:
#se.get_position('PETR4')
#pos=se.get_position('ITUse')
...
#def adjust_weights(assets,w,capital):
   # sum=0
    ...
    #    shares=se.get_affor_shares(asset,aval_capital,last_price)
    #    if shares<=0:
    #        w[asset]=0
    #    else:
