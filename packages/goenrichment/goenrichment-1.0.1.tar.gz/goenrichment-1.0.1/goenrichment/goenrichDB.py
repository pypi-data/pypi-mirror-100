...
def main():
    ...
    parser.add_argument('--tsv',
                        help='TSV file with at least two columns: Gene_name<tab>GO terms',
                        required=False)
    parser.add_argument('--goenrichDB', help='Previous created goenrich pickle file. '
                                             'The new genes will be added to this database',
                        required=False)
    parser.add_argument('--goa_uniprot', help='Uniprot GOA file GAF format',
                        required=False)
    parser.add_argument('--gobo', help='UGO Obo file from Gene Ontology',
                        required=False)
    parser.add_argument('--taxid', help='Process genes for tax id if it is possible',
                        required=False)
    ...
    if gene_info_file and gene2go_file:
        ...
        values = load_ncbi_gene(geneGoDB, gene_info_file,
                                gene2go_file, tax_id)
