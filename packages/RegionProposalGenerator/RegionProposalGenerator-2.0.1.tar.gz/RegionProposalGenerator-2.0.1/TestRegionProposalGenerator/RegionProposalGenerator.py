...
__date__      = '2021-March-31'   
__url__       = 'https://engineering.purdue.edu/kak/distRPG/RegionProposalGenerator-2.0.1.html'
...
CHANGE LOG:
  ...
  Version 2.0.1:
    ...
    one goes from since-instance object detection to multi-instance object
...
INTRODUCTION:
    ...
    Single-Instance vs. Multiple-Instance Detections:
...
USAGE:
    ...
    Single-Instance and Multiple-Instance Detections:
...
Inner Classes:
    ...
    (1)  PurdueDrEvalDataset
         ...
         and colored and substantial structured noise in addition to 20%
         Gaussian noise.  Examples of these images are shown in Week 9
         lecture material in Purdue's Deep Learning class.

    (2)  PurdueDrEvalMultiDataset

         This is the dataset to use if you are experimenting with
         multi-instance object detection.  Each image in the dataset
         contains randomly chosen multiple instances of same three kinds of
         objects as mentioned above: Dr. Eval, house, and watertower.  The
         number of object instances in each image can be no more than five.
         The images contain substantial amount of structured noise besides
         20% random noise.
         ...
         One of three objects types in these images a human-like cartoon
         figure that I needed a name for.  So, after Dr. Evil in the
         movies, I decided to call this cartoon figure Dr Eval and to refer
         to the datasets as Dr Eval datasets.

    (3)  SingleInstanceDetector

         This provides a reference implementation for constructing a
         single-instance object detection using the PurdueDrEvalDataset
         dataset. For the detection and regression network, it uses the
         LOADnet2 network from DLStudio with small modifications to account
         for the larger 128x128 images in the PurdueDrEvalDataset dataset.

    (4)  YoloLikeDetector

         The code in this inner class provides an implementation for key
         elements of the YOLO logic for multi-instance object detection.
         Each training image is divided into a grid of cells and it is the
         responsibility of a cell that contains the center of an object
         bounding box to provide at the output of the neural network an
         estimate for the exact location of the center of the object
         bounding box vis-a-vis the center of the cell.
...
class RegionProposalGenerator(object):
    ...
    class YoloLikeDetector(nn.Module):             
        ...
        current version 0 implementation, I have not bothered with the 8 rows at the
        bottom of the image and the 8 columns at the right border that would not left
        out of the pixels covered by such a grid.]
            ...
            class AnchorBox:
                ...
                def __str__(self):
                    ...
                    return "AnchorBox type (h/w): %s    tlc for yolo cell: %s    anchor-box height: %d     \
                                     anchor-box width: %d" % (self.AR, str(self.tlc), self.ab_height, self.ab_width)
            for epoch in range(self.rpg.epochs):  
                print("")
                running_loss = 0.0
                for iter, data in enumerate(self.train_dataloader):   
                    ...
                    for ibx in range(im_tensor.shape[0]):
                        for idx in range(num_objects_in_image[ibx]):
                            ...
                            yolo_tensor[ibx, yolo_cell_index, idx] = torch.FloatTensor( yolo_vector )
                    if yolo_debug:
                        logger = logging.getLogger()
                        old_level = logger.level
                        logger.setLevel(100)
