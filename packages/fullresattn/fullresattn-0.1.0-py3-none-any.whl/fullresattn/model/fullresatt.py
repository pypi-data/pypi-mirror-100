...
class fullresAttCoder:

    def __init__(self,nfea,layernNodes,acts=None, extranOutput=0, k_initializer='he_normal',dropout=0.5, batchnorm=True, reg=None, inresidual=False, outnres=None, defact=None, outputtype=0, nAttlayers=None, *args, **kwargs):
        """
        ...
        :param nAttlayers: integer, the number of the attention layers (default: None, no attention layers added).
...
#            layer = add([orginlayer, layer])
    ...
    def resAutoNet(self):
        ...
        layer=inputlayer
        if self.nattlayers is not None: 
          # ATTENTION PART STARTS HERE 注意力层
          for i in range(self.nattlayers):
            inlayer=layer
            attention_probs = Dense(self.nfea, activation='softmax', name='attention_vec'+str(i))(inlayer)
            layer =  Multiply()([layer, attention_probs])
            layer=add([layer,inlayer])
            layer = BatchNormalization()(layer)

        if self.outnres is None or self.outnres==0:
            layer = self.levelsBlock(layer,0, self.dropout)
        else:
            layer = self.basicResUnit(layer, self.outnres)
