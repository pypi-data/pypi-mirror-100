    ...
    while cost > tol and runtime_statistics["status"] == -1:
        ...
        if preconditioning:
            if callable(preconditioner_factory):
                M = preconditioner_factory(
                    orbit_instance, "lstsq", **kwargs
                )
                ...
                eye = lambda x: x
                M = LinearOperator((A.shape[1], A.shape[1]), matvec=eye, rmatvec=eye, matmat=eye, rmatmat=eye)
...
def _scipy_optimize_root_wrapper(
    ...
    while cost > tol and runtime_statistics["status"] == -1:
        ...
        _rootfunc, jac_and_jac_options = func_jac_factory(orbit_instance, method, **kwargs)
        scipy_kwargs = {
            "tol": tol,
