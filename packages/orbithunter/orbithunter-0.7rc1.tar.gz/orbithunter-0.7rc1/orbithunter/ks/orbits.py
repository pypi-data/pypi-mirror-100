...
class OrbitKS(Orbit):
    ...
    def preconditioner(self, **kwargs):
        ...
        def matmat_(V):
            ...
            return diag_M[:V.shape[0], :].reshape(-1, 1) * V

        # rmatvec = matvec because diagonal
        return LinearOperator(
            shape=(diag_M.size, diag_M.size), matvec=matvec_, rmatvec=matvec_, matmat=matmat_, rmatmat=matmat_
        )
    ...
    def _inv_time_transform(self, array=False, inplace=False):
        ...
        if inplace:
            ...
            modes[-1, :] = 0
            space_modes = irfft(
                modes, workers=self.workers, norm="ortho", axis=0
            )
            if array:
                return space_modes
            else:
...
class AntisymmetricOrbitKS(OrbitKS):
    ...
    def to_fundamental_domain(self, half=0, **kwargs):
        ...
        """
        if half == 0:
            return self.transform(to="field")[:, : -int(self.m // 2)].transform(to=self.basis)
        else:
            return self.transform(to="field")[:, -int(self.m // 2):].transform(to=self.basis)
...
class ShiftReflectionOrbitKS(OrbitKS):
    ...
    def to_fundamental_domain(self, half=0):
        ...
        """
        if half == 0:
            return self.transform(to="field")[-int(self.n // 2) :, :].transform(to=self.basis)
        else:
            return self.transform(to="field")[:-int(self.n // 2), :].transform(to=self.basis)
