...
class Orbit:
        ...
        try:
            ...
            if self.parameters is not None:
                ...
                return self.__class__(
                    **{**vars(self), "state": state_slice, "parameters": parameters, "discretization": None}
                )
            else:
                return self.__class__(**{**vars(self), "state": state_slice, "discretization": None})
        except IndexError as ie:
            if self.size == 0.0:
                raise ValueError(
    ...
    def concat(self, other, axis=0):
        ...
        if self.parameters is not None and other.parameters is not None:
            ...
            new_dimensions = self.glue_dimensions(tuple_of_zipped_dimensions, glue_shape, exclude_nonpositive=True)
        ...
        if new_dimensions is not None:
            ...
            concat_orbit = self.__class__(
                **{**vars(self), "state": concatenated_state, "parameters": parameters, "discretization": None}
                )
        else:
