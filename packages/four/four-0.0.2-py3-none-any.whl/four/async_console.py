...
def get_secret():
    ...
    return ''.join(secrets.choice(alphabet) for i in range(24)).encode('utf-8')
...
class Executor():
    ...
    def exec_nodes(self, nodes, exception):
        try:
            compiled = compile(ast.Module(body=nodes), filename='<executor>', mode='exec')
    ...
    def eval_node(self, node, exception):
        try:
            compiled = compile(ast.Expression(body=node.value), filename='<executor>', mode='eval')
    ...
    def execute(self, code):
        threading.Thread(target=self.exec_and_update_handler, args=(code,), name='Executor').start()
...
def start(globals, return_server=False):
    ...
    threading.Thread(target=serve, args=(handler,)).start()
