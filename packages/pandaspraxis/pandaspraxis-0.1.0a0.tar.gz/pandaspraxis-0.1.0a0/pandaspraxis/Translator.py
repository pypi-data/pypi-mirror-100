    ...
    def select(self, select):
        ...
        if contains('functionfields', children, recurse=False) and \
            contains('function', children, recurse=True):
                ...
                funct_or_tree = Tree('or_node', [make_command(funct_opt1, table_name), make_command(funct_opt2, table_name)])
        ...
        if contains('functionfields', children, recurse=False) or \
            ...
            if len(fields_children) == 1 and fields_children[0] == Tree('allfields', []):
                ...
                if len(simple_children) == 1:
                    ...
                    fields_tree = Tree('or_node', [make_command(fields_tree, table_name), make_command(fields_tree2, table_name)])
        ...
        if contains('groupby', children, recurse=False):
            ...
            if final_nodes['function'] is None:
                ...
                groupby_opt1 = Tree('groupby', [groupby_tree, funct_opt1])
                groupby_opt2 = Tree('groupby', [groupby_tree, funct_opt2])
...
class PandasUnparser(TreeTransformer):
    ...
    def groupby(self, tree):
        ...
        if isinstance(tree, str): #TODO this is bad
            return tree
    ...
    def or_node(self, tree):
        ...
        # for now, just move this from a tree format to a json/dict format;
        # later we'll choose between options based on user knowledge + re-format for user viewing

        main_concept = next(iter(tree.concept)) # tree.concept is a one-key dict
        options = {tree.concept[main_concept][i]: tree.children[i] for i in range(len(tree.children))}
        choices = {'options': options, 'tooltip': tree.info, 'concept': main_concept, \
            'meta': (tree.column if tree.column is not None else -1,
