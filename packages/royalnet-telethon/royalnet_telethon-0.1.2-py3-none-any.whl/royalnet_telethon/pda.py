...
class TelethonPDA:
    ...
    def __init__(self,
        ...
        self.me: t.Optional[tlt.User] = None
    ...
    def _determine_key(self, event: tlc.message.Message):
        if self.mode == TelethonPDAMode.GLOBAL:
            ...
            return event.chat_id, event.from_id
        else:
    ...
    def register_partial(self, part: engi.PartialCommand, names: t.List[str]) -> engi.Command:
        ...
        if part.syntax:
            command = part.complete(pattern=r"^/{name}(?:@{botname})?\s+{syntax}$", names=names, botname=self.me.username)
        else:
            command = part.complete(pattern=r"^/{name}(?:@{botname})?$", names=names, botname=self.me.username)
