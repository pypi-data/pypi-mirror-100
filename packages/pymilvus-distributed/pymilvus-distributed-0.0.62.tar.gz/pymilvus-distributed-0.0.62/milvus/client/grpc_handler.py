...
class GrpcHandler(AbsMilvus):
    ...
    def _divide_search_request(self, collection_name, query_entities, partition_tags=None, fields=None, **kwargs):
        to = kwargs.get("timeout", None)
        rf = self._stub.HasCollection.future(Prepare.has_collection_request(collection_name), wait_for_ready=True,
                                             timeout=to)
        reply = rf.result()
    ...
    @error_handler(None)
    def _batch_search(self, collection_name, query_entities, partition_tags=None, fields=None, **kwargs):
        to = kwargs.get("timeout", None)
    ...
    @error_handler(None)
    def _total_search(self, collection_name, query_entities, partition_tags=None, fields=None, **kwargs):
        to = kwargs.get("timeout", None)
    ...
    def search(self, collection_name, query_entities, partition_tags=None, fields=None, **kwargs):
        if kwargs.get("_deploy_mode", DeployMode.Distributed) == DeployMode.StandAlone:
            return self._total_search(collection_name, query_entities, partition_tags, fields, **kwargs)
