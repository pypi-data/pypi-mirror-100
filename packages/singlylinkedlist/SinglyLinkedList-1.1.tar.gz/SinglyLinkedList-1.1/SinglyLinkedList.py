...
class List:
    ...
    def pop(self, index='None'):
        if not (type(index) == int or index == 'None'):
            raise TypeError
        if index == 'None':
            index = self.size - 1
    ...
    def deepcopy(self):
        ...
        node = self.head
        while node:
            if type(node.data) in [int, float, complex, str, bytes, tuple]:
                copied_list.append(node.data)
            elif type(node.data) == List:
                copied_list.append(node.data.deepcopy())
            else:
                copied_list.append(deepcopy(node.data))
            node = node.next
        return copied_list
