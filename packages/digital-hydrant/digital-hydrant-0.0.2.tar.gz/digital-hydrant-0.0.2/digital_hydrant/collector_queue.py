...
class CollectorQueue:
    def __init__(self, db_name=None):
        ...
        try:
            ...
            columns = "id INTEGER PRIMARY KEY AUTOINCREMENT, type TEXT, payload TEXT, timestamp TIMESTAMP, uploaded INTEGER DEFAULT 0"
            self.cursor.execute(
                f"CREATE TABLE IF NOT EXISTS {self.__table__} ({columns})"
            )
        except Exception as err:
            self.logger.critical(f"Failed to initialize database: {err}")
            exit(1)
