...
class TestCollectorQueue(unittest.TestCase):
    def setUp(self):
        ...
        self.queue = CollectorQueue(self.database_file)
    ...
    def test_put(self):
        ...
        cursor.execute("SELECT id, type, payload, timestamp, uploaded FROM collectors")

        (id, type, payload, timestamp, uploaded) = cursor.fetchone()
    ...
    def test_remove(self):
        ...
        (count,) = cursor.execute("SELECT COUNT(*) FROM collectors").fetchone()
        self.assertEqual(count, 2)
        self.queue.remove(1)
        (count,) = cursor.execute("SELECT COUNT(*) FROM collectors").fetchone()
        self.assertEqual(count, 1)

        (id, uploaded) = cursor.execute(
            "SELECT id, uploaded FROM collectors"
        ).fetchone()
        self.assertEqual(uploaded, 0)
        self.queue.remove(2, delete=False)
        (count,) = cursor.execute("SELECT COUNT(*) FROM collectors").fetchone()
        self.assertEqual(count, 1)
    ...
    def test_remove_all_no_delete(self):
        ...
        data = cursor.execute("SELECT id, uploaded FROM collectors").fetchall()
