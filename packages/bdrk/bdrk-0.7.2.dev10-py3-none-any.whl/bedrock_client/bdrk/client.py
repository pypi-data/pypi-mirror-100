...
from bdrk.v1.models.create_model_version_schema import CreateModelVersionSchema
from bdrk.v1.models.model_version_schema import ModelVersionSchema
...
class BedrockClient(object):
    ...
    def create_model_version(
        ...
        response = self._model_api.create_model_version(
            project_id=self._project_id,
            model_id=model_id,
            create_model_version_schema=CreateModelVersionSchema(
    ...
    @bedrock_env_skipped
    def download_and_unzip_model_version(self, model_id: str, version: int, path: str):
        """Zips and uploads a trained model. This should be called inside a run context.

        Args:
            model_id (str): model id
            version (int): model version number
            path (str): Destination path where model version artefacts are downloaded to.
            If unspecified, will download to the current working directory
        """
