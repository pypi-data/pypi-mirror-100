...
from . import ClassifierFairness, RegressionFairness
...
class ModelAnalyzer:
    def __init__(
        ...
        model_task: Optional[ModelTask] = ModelTask.CLASSIFICATION,
        description: str = "",
    ):
    ...
    def analyze(self):
        ...
        if self.fconfig:
            if self.test_feat is None or self.test_lbs is None or self.test_inf is None:
                ...
                fairness = (
                    RegressionFairness if self.model_task is ModelTask.REGRESSION else ClassifierFairness
    ...
    def _log_fai_data(
        ...
        if self.api.has_api_settings:
            ...
            if len(output):
                output["fairness_type"] = model_task
