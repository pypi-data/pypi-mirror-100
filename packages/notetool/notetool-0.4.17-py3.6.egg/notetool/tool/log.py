...
def get_lapse_time(run_time):
    ...
    if gap_h > 0:
        gap_time = "%dh%dm%ds" % (gap_h, gap_m, gap_s)
    elif gap_m > 0:
        gap_time = "%dm%ds" % (gap_m, gap_s)
    else:
        gap_time = "%ds" % gap_s
...
class LogTool:
    def __init__(self):
        ...
        self.record_level = 'debug'

    def reset_time(self, msg, level):

        self.record_msg = msg
        self.record_time = time.time()
        self.record_level = level

    def _run(self, level, msg, run_time=None, *args, **kwargs):
    ...
    def run(self, level, msg, record=True, before=False, *args, **kwargs):
        if before:
            run_time = time.time() - self.record_time
            self._run(level, msg, run_time, *args, **kwargs)
            self.reset_time(msg, level)

        elif record:
            run_time = time.time() - self.record_time
            self._run(self.record_level, self.record_msg, run_time, *args, **kwargs)
            self.reset_time(msg, level)
            self._run(level, msg, *args, **kwargs)
