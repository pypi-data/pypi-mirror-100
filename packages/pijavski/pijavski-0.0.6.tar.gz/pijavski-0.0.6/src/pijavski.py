...
def get_minimum(func=lib.fun1, lip=2., xl=-50000., xu=90000., precision=1e-12, maxiter=300000):
    """
    ...
    The function to optimise needs to be declared as a callback function 
    for CFFI so that the Pijavski program can process it. User can implement 
    up to 20 functions following the conditions below:
    
    - The function definition needs to be preceeded by `@ffi.def_extern()`.
    
    - The function name must be `fun[1-20]` (suffix between 1 and 20) as this 
    is how the callback function is defined in the CFFI builder.
    
    - When writing the function,  arguments `f` and `x` need to be declared 
    as if they were pointers using the bracket notation f[] and x[].
    ...
    0 0.0 -1.0 1.9431114196777344e-05 65533
    >>> # Call get_minimum for fun2
    >>> get_minimum(func=lib.fun2, xl=-100, xu=100)
    0 0.0 0.0 1.9376064301468432e-05 65533
    """
