...
class SkeletonVisualizer:
    ...
    def visualize_video(
        ...
        self.__assert_skeleton_shapes(frames[0])
        colors = self.__init_colors(frames[0].shape[0], colors)

        self.plot = k3d.plot(camera_auto_fit=False)
        self.__adjust_camera_starting_position(frames[0], self.plot)
        skeleton_part_size = self.__calculate_skeleton_part_size(frames[0])

        frames = np.swapaxes(frames, 0, 1)
        skeleton_timestamps: List[Dict[str, np.ndarray]] = [
            {
                str(timestamp_index): frames[current_frame_index][timestamp_index]
                for timestamp_index in range(len(frames[current_frame_index]))
            }
            for current_frame_index in range(len(frames))
        ]

        for skeleton in map(
                lambda skeleton_color_tuple: Skeleton(
                    joint_positions=skeleton_color_tuple[0],
                    joint_set=self.joint_set,
                    part_size=skeleton_part_size,
                    color=skeleton_color_tuple[1]),
                zip(skeleton_timestamps, colors)):
            drawable_skeleton = skeleton.to_drawable_skeleton_for_video()
    ...
    def __create_skeleton_plot(self, skeletons: np.ndarray,
        ...
        skeleton_plot = k3d.plot(camera_auto_fit=False)
