...
class SkeletonVisualizer:
    ...
    def __create_skeleton_plot(self, skeletons: np.ndarray,
        ...
        skeleton_plot = k3d.plot(antialias=0, background_color=0xDDDDDD)
        centroid = self.__calculate_centroid(skeletons)
        skeleton_plot.camera = [0, 0, 0, centroid[0], centroid[1], centroid[2], 1, 0, 0]
        for skeleton in map(
