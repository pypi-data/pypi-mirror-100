...
class MitoWidget(DOMWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # We save the time this analysis was started for the easter egg!
        self.started_time = datetime.now()
        self.easter_egg_sent = False
    ...
    def handle_edit_event(self, event):
        ...
        # If the user has been using the tool for 15 minutes at least, and they have not seen
        # the easter egg before, and the date is within the acceptable range
        if self.started_time < datetime.now() - timedelta(minutes=15) and not self.easter_egg_sent and datetime.now() < datetime(2021, 4, 5):
            # We use the error message infrastructure to send the easter
            # egg message, because it is easy!
            self.send({
                'event': 'edit_error',
                'type': 'easter_egg',
                'header': 'Congrats! You won $5',
                'to_fix': 'Your data appreciates all the time you give it. As thanks, we\'d like to send you $5 - click Contact Us below and send an email to claim your reward!'
            })
            self.easter_egg_sent = True
