...
def build_dockerfile_from_template(mito_version, on_staging) -> str:
    """
    ...
    if on_staging:
        with open('deployment/templates/staging_docker_template.txt', 'r') as f:
...
def build_and_upload_docker_image(mito_version, on_staging):
    """
    ...
    """
    dockerfile_string = build_dockerfile_from_template(mito_version, on_staging)
...
def deploy_current_mito_version_to_pypi(on_staging):
    """
...
def main():
    ...
    run this script with `python3 deployment/deploy.py [staging | app]`.
    ...
    try:
        ...
        deploy_current_mito_version_to_pypi(on_staging)
    ...
    docker_tag = build_and_upload_docker_image(mito_version, on_staging)
