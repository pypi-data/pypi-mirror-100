...
class Expression:
    ...
    def dice_per_face(self):
        output = {}
        results = self.__dice_tosses[::2]
        faces = self.__dice_tosses[1::2]
        for face in faces:
            if face not in output.keys():
                output[face] = []
        for i in range(len(results)):
            output[faces[i]].append(results[i])
        return output
    ...
    def __dice_solver(self, string, operand_solver):
        ...
        if faces == 20:
            if result == 1:
                return result, 1
            elif result == 20 and (dice_number == 1 or (dice_number == 2 and ad_flag != 0)):
                return result, 2
        return result, 0
