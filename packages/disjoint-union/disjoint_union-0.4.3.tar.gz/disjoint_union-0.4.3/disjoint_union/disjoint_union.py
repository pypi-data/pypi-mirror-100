...
from typing import Optional, Union, Any, List, \
  Hashable, Set, Iterable, Sequence
...
#   return self.__add__(other)
  ...
  def combine_new(self, other: Union[DisjointUnion, Any]) -> DisjointUnion:
    if isinstance(other, type(self)):
  ...
  def unions(self, *many_items: Hashable) -> DisjointUnion:
    single_item = len(many_items) == 1

    if single_item:
      item = many_items[FIRST_ITEM]
      return self.union_iterable(item)
