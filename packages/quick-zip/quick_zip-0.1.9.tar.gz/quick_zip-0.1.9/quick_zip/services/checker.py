...
def audit(destination: Path, oldest_by_days: int) -> Audit:
    ...
    all_files = [BackupFile(path=file, days_old=fstats.get_days_old(file)) for file in destination.iterdir()]

    all_files.sort(key=lambda x: x.days_old)

    newest: BackupFile = all_files[0]
    oldest: BackupFile = all_files[-1]
    health = newest.days_old < oldest_by_days

    my_cards = []
    for file in all_files:
        file: BackupFile
        health = file.days_old < oldest_by_days
