...
from sob.model import Array, Dictionary, Model, Object, detect_format, unmarshal
        ...
        for index in range(len(array)):
            ...
            if isinstance(item, Reference):
                array[index] = self.resolve(
                    item.ref, types=array_meta.item_types, dereference=recursive
                )
        ...
        for key, value in dictionary.items():
            if isinstance(value, Reference):
                dictionary[key] = self.resolve(
                    value.ref, types=dictionary_meta.value_types, dereference=recursive
                )
            elif recursive and isinstance(value, Model):
                self.dereference(value, recursive=recursive)
