...
class _Modeler:
    ...
    def set_relative_url_pointer_class_name(self, relative_url_pointer, class_name_):
        ...
        if relative_url_pointer in self._relative_urls_pointers_class_names:
            class_name_ = self._relative_urls_pointers_class_names[relative_url_pointer]
        else:
    ...
    def get_property(
        ...
        if (
            ...
            property_ = properties.Property(required=required)
            property_types = self.get_model(schema, is_property=(not is_referenced))
        ...
        if schema.enum:
            property_ = properties.Enumerated(
                values=tuple(schema.enum), types=(property_,), required=required
            )
        if name is not None:
            property_.name = name
        if (
    ...
    def get_schema_array(
        self, schema, name=None, relative_url_pointer=None, required: bool = False
    ):
        ...
        if items_schemas:
            ...
            for items_schema in items_schemas:
                if schema_defines_model(items_schema):
                    items_model = self.get_model(items_schema, required=required)
                else:
    ...
    def get_schema_dictionary(
        ...
        if isinstance(schema.additional_properties, (Schema, Reference)):
            ...
            name = self.set_relative_url_pointer_class_name(relative_url_pointer, name)
    ...
    def get_schema_object(self, schema, name=None, relative_url_pointer=None):
        ...
        for name_, property_ in schema.properties.items():
            ...
            object_meta.properties[property_name_] = self.get_property(
                ...
                required=(
                    True if (schema.required and (name_ in schema.required)) else False
                ),
            )
        name = self.set_relative_url_pointer_class_name(relative_url_pointer, name)
    ...
    def get_relative_url_and_pointer_class_name(
        ...
        if not self.relative_url_pointer_class_name_exists(relative_url_pointer):
            parts = [
                part.replace("~1", "/") for part in pointer.lstrip("#/").split("/")
            ]
            length = len(parts)
            start_index = 0
    ...
    def _resolve(self, model_instance, types=None):
        ...
        if isinstance(model_instance, Reference):
            ...
            while isinstance(model_instance, Reference):
                resolved_model_instance = self.resolver.get_document(url).resolve(
                    pointer, types
                )
    ...
    def module_definition(self) -> str:
        ...
        return "\n".join(
            sorted(imports, key=lambda sort_line: 1 if sort_line == "import sob" else 0)
            + ["\n"]
            + classes
            + [
