...
class Sudoku:
    ...
    def _check_one_possible(self):
        ...
        for r, c in self.base_points:
            # reshape: 3x3 改为1维数组
            values = list(filter(lambda x: isinstance(x, list),
...
def solve2(data):
    t1 = time.time()
    res = sudoku_solve_solution2(data)
    t2 = time.time()
...
def solve1(data):
    t1 = time.time()
    res = sudoku_solve_solution1(data)
    t2 = time.time()
...
if __name__ == "__main__":
    ...
    solved1 = solve2(data1)
    solved2 = solve1(data1)
