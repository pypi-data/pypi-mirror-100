...
def _load_benchmark(
    ...
    benchmarks = []
    dir_path = os.path.dirname(actual_tt_json_path)
    ...
    for model_name, stat in tqdm(list(actual_tt_json.items())[:upper_limit_per_type]):
        ...
        bmdata.model_info["raw_model"] = tf.keras.models.load_model(model_path)
        training_size = stat["training_size"]
        batch_input_shape = bmdata.model_info["raw_model"].get_config()["layers"][0]["config"]["batch_input_shape"]
...
if __name__ == "__main__":
    def get_feature_func(model, x_shape, kwargs):
        ...
        df = convertModelToRawData(model, training_size)
