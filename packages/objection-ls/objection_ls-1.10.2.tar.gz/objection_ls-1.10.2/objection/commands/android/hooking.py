...
def show_android_classes(args: list = None) -> None:
    ...
    if len(clean_argument_flags(args))  < 1:
        ...
        if "--exclude" in args:
            ...
            include_args = args[:index]
            exclude_args = args [index+1:]
        else:
             include_args = args
             exclude_args = []
        ...
        for class_name in sorted(classes):
            for include in include_args:
                if(class_name.find(include)>=0):
                    check = 1
            
            if (exclude_args!=[]):        
                for exclude in exclude_args:
                    if(class_name.find(exclude)>=0):
                        check = 0
...
def watch_class(args: list) -> None:
    ...
    if "--hooking" in args:
        ...
        parse_inputs = []
        methods = api.android_hooking_get_class_methods(target_class)
        for method in methods:
            # method = public native boolean org.json.JSONObject.getBoolean(java.lang.String)
            parse_string = method.split(' ')[3].replace('(',' ').replace(')','')
            parse_inputs.append(parse_string)
        print("Parse_inputs: ", parse_inputs)

        for parse_input in parse_inputs:
            # parse_input = o.k.b android.content.Context,java.lang.String
            clase_method = parse_input.split(' ')[0]
            overload =  parse_input.split('     ')[1]
            watch_class_method([clase_method, overload, "--dump-args", "--dump-retur"])
