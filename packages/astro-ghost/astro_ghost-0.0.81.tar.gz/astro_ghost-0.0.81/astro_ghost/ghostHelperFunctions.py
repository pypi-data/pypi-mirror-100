...
from astro_ghost.DLR import calc_DLR, chooseByDLR
import requests
import pickle
import os
import glob
...
#         printout
def getTransientHosts(snName=[''], snCoord=[''], snClass=[''], verbose=0, starcut='normal', gradientAscent=True):
    hostDB = None
    ...
    if tempHost1 is None or len(notFoundNames) > 0:
        ...
        if tempHost2 is None or len(notFoundCoords) > 0:
            ...
            tempHost3 = findNewHosts(snName_remaining, snCoord_remaining, snClass_remaining, verbose, starcut, gradientAscent)
...
#         with stats provided at
#         printout
def findNewHosts(snName, snCoord, snClass, verbose=0, starcut='gentle', ascent=True):
    ...
    #plotLocus(host_gals_DF, color=1, saae=1, type="Gals", timestamp=dateStr)
...
#    fracWithHosts(host_dict_nospace)
    ...
    if (len(SN_toReassociate) > 0) and (ascent):
        if verbose:
        ...
        if verbose:
            if len(unchanged) != 1:
                ...
                print("Hosts not found for %i transient in gradient ascent. Storing names in GD_unchanged.txt" %(len(unchanged)))
    ...
    # re-calculate values for the SN matched by gradient ascent, to be used for classification
    #DLR = []
    #dist = []
    host_DF = getColors(host_DF)
    host_DF = calc_7DCD(host_DF)

    host_DF['gApMag_gKronMag'] = host_DF['gApMag'] - host_DF['gKronMag']
    host_DF['rApMag_rKronMag'] = host_DF['rApMag'] - host_DF['rKronMag']
    host_DF['iApMag_iKronMag'] = host_DF['iApMag'] - host_DF['iKronMag']
    host_DF['zApMag_zKronMag'] = host_DF['zApMag'] - host_DF['zKronMag']
    host_DF['yApMag_yKronMag'] = host_DF['yApMag'] - host_DF['yKronMag']
    ...
    for idx, row in host_DF.iterrows():
        if row.dist != row.dist:
            try:
                theta = np.nan
                #get source
                table1 = host_DF.iloc[[idx]]
                tempDist, R = calc_DLR(Angle(table1['TransientRA'].values[0]*u.deg),Angle(table1['TransientDEC'].values[0]*u.deg),table1['raMean'], table1['decMean'], np.float64(table1['rKronRad']), np.float64(table1['rKronRad']), table1, 'r')
                host_DF.loc[idx, 'dist'] = tempDist[0]
                host_DF.loc[idx, 'dist/DLR'] = R
            except:
                host_DF.loc[idx, 'dist'] = np.nan
                host_DF.loc[idx, 'dist/DLR'] = 1.e10
