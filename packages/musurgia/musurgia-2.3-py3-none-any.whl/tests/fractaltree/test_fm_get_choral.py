...
class Test(AGTestCase):
    ...
    def test_1(self):
        ...
        for leaf in self.fm.traverse_leaves():
            leaf.set_chord(leaf.get_multi_chord(range_factor=1.5, direction=1, last=True))
    ...
    def test_2(self):
        ...
        for leaf in self.fm.traverse_leaves():
            leaf.set_chord(leaf.get_multi_chord(range_factor=1, direction=1, last=False))
        sf = self.fm.get_simple_format()
    ...
    def test_3(self):
        ...
        expected = [child.get_multi_chord_midis() for child in fm.get_children()]
        actual = [child.get_multi_chord_midis() for child in fm.get_children()]
