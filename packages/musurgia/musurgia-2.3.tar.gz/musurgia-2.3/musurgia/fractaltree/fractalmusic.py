...
class FractalMusic(FractalTree):
    ...
    def _validate_multi_chord_number_of_midis(self, number_of_midis):
        max_number_of_midis = len(self.children_generated_midis)
        if number_of_midis is not None and not max_number_of_midis >= number_of_midis > 0:
            raise ValueError(f'Bad number_of_midis: {number_of_midis}')
    ...
    def get_multi_chord_midis_with_range_factor(self, range_factor=1, number_of_midis=None, microtone=2,
                                                no_duplicates=False, original_direction=False):
        self._validate_multi_chord_number_of_midis(number_of_midis)

        if not original_direction:
            range_interval = abs(self.midi_generator.midi_range[1] - self.midi_generator.midi_range[0]) * range_factor
        else:
            range_interval = (self.midi_generator.midi_range[1] - self.midi_generator.midi_range[0]) * range_factor

        return self.get_multi_chord_midis_with_range_interval(range_interval, microtone=microtone,
                                                              number_of_midis=number_of_midis,
                                                              no_duplicates=no_duplicates)

    def get_multi_chord_midis_with_range_interval(self, range_interval, number_of_midis=None, microtone=2,
                                                  no_duplicates=False):
        self._validate_multi_chord_number_of_midis(number_of_midis)
        copied_node = self.__deepcopy__()
        copied_node.midi_generator.midi_range = self.midi_value, self.midi_value + range_interval
        copied_node.midi_generator.microtone = microtone
        if number_of_midis is None:
            copied_node.add_layer()
        else:
            copied_node.generate_children(number_of_children=number_of_midis - 1)
        multi_chord_midis = sorted(copied_node.children_generated_midis)
        if no_duplicates:
            multi_chord_midis = dict.fromkeys(multi_chord_midis)
        return multi_chord_midis
    ...
    def calculate_range_factor(self, midi_range_interval):
        if midi_range_interval == 0:
            raise ValueError(f'Bad midi_range_interval: {midi_range_interval}')
        midi_range_values = self.midi_generator.midi_range
        original_midi_range_interval = midi_range_values[1] - midi_range_values[0]
        if original_midi_range_interval == 0:
            raise ValueError(f'Bad original_midi_range_interval: {original_midi_range_interval}')
        return midi_range_interval / original_midi_range_interval
    ...
    def __deepcopy__(self, memodict={}):
        ...
        copy_attribute_names = ['_chord_field', '_chord', '_midi_value', '_simple_format', '_tree_directions',
                                '_permute_directions', '_tempo',
                                '_instrument', '_staff_number']

        self.deepcopy_attributes(copied, copy_attribute_names)
        copied.midi_generator = self.midi_generator.__deepcopy__()
