...
def shipment_date_range(shipment_tuple: ShipmentTimelineInfos, shipment_tasks: pd.Series) -> pd.DatetimeIndex:
    start_date = get_shipment_start_date(shipment_tuple, shipment_tasks)
...
def shipments_timelines(active_shipments_data_df: pd.DataFrame, tasks_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for shipment_timeline_infos_tuple in shipments_timeline_infos_grouped.itertuples(True, "ShipmentTimelineInfos"):
        ...
        dates = shipment_date_range(shipment_tuple=shipment_timeline_infos_tuple, shipment_tasks=shipment_tasks)
...
def shipments_daily_workload_timelines(active_shipments_data_df: pd.DataFrame, tasks_df: pd.DataFrame) -> pd.DataFrame:
    ...
    shipments_timelines_df = shipments_timelines(active_shipments_data_df, tasks_df)
