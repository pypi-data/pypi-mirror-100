...
app = Typer(context_settings=appcontext)
...
class Git(git.Repo, BaseData):
    ...
    def all(self, bump: Bump = Bump.PATCH, keep: bool = GIT_VERSIONS, message: str = None,
        ...
        if self.git_dir:
            ...
            return self.bump(bump=bump, keep=keep, message=message, stdout=stdout)
    ...
    def bump(self, bump: Bump = Bump.PATCH, keep: bool = GIT_VERSIONS, message: str = None,
        ...
        if self.git_dir:
            ...
            self.version_delete(keep=keep, stdout=stdout)
...
class Package(Path):
    ...
    __slots__ = ('_file', '_init_py', '_path', '_venv', 'app', 'bapy', 'context', 'env', 'frame', 'git', 'ic', 'icc', 'is_git', 'kill',
                 'log', 'logconf', 'package', 'prefix', 'sem', 'semfull', 'work', 'kwargs', )
    ...
    def __repr__(self):
        return f'{self.__class__.__name__}(f"{self.__class__.__name__}({self})", \n' \
               ...
               f'_venv {slot(self, "_venv")}, \n'\
               f'app: {slot(self, "app")}, \n'\
               f'bapy: {slot(self, "bapy")}, \n'\
               f'context: {asdict(slot(self, "context"))}, \n'\
               f'env: {slot(self, "env")}, \n'\
               f'file: {self.file}, \n'\
               f'git: {slot(self, "git")}, \n'\
    ...
    def all(self, bump: Bump = Bump.PATCH, clear: bool = VENV_CLEAR, deps: bool = VENV_DEPS,
            keep: int = GIT_VERSIONS, message: str = None, stdout: bool = STDOUT):
        if self.is_git:
            self.project.c()
            self.venv(clear=clear, deps=deps, stdout=stdout)
            self.deps(stdout=stdout)
            version = self.git.all(bump=bump, keep=keep, message=message, stdout=stdout)
            self.clean(stdout=stdout)
            return self.setup(version=version, stdout=stdout)
            self.upload(stdout=stdout)
            self.clean(stdout=stdout)
            self.install(stdout=stdout)
            self.project.c_()
    ...
    def deps(self, stdout: bool = STDOUT):
        if self._venv:
            self._alt(stdout=stdout)
    ...
    def _init_obj(cls, file: str, frame: FrameInfo) -> Package:
        ...
        obj.is_git = bool(obj.git.git_dir)
        obj._venv = (obj.git.path / VENV_DIR) if obj.is_git else None
        obj.context = Simple if obj.is_git else None  # python_exe, libpath, bin_path, env_exe
        obj.app = Typer(name=obj.name, context_settings=appcontext)
        return obj
    ...
    def install(self, stdout: bool = STDOUT):
        if self.is_git:
            self._alt(stdout=stdout)

    def manifest(self, stdout: bool = STDOUT):
        if self.is_git:
            (self.project / MANIFEST).write_text('\n'.join([f'include {l}' for l in self.git.ls]))
            self._alt(stdout=stdout)
    ...
    def setup(self, version: str, stdout: bool = STDOUT) -> dict:
        if self.is_git:
            slot(self, 'kwargs', dict(
                author=user.gecos, author_email=Url.email(), description=self.description,
                entry_points=dict(console_scripts=[f'{p} = {p}:{self.name}.{CLI}' for p in self.packages_upload]),
    ...
    def upload(self, stdout: bool = STDOUT):
        if self.is_git:
            self._alt(stdout=stdout)
    ...
    def venv(self, clear: bool = VENV_CLEAR, deps: bool = VENV_DEPS, stdout: bool = STDOUT):
        if self._venv:
            venv = EnvBuilder(clear=clear, upgrade_deps=deps)
            exists = self._venv.exists()
            # noinspection PyAttributeOutsideInit
            self.context = venv.ensure_directories(self._venv.text)
            ic(self)
            if not exists or clear:
                venv.create(self._venv.text)
            if deps:
                venv.upgrade_dependencies(self.context)
            self._alt(stdout=stdout)
...
def load_modules(p: str = None) -> None:
    ...
    Args:
        p: package.
    """
    p = p if p else package.package.name
    p._modules = []
