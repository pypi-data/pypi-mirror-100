...
if(a != ""):
    try:							#시도하라
        arduino = serial.Serial("com%s"%a,115200)		#아두이노와 시리얼 연결을 시작하라(1번포트부터 100까지)
        print("아두이노 연결 성공")				#만약 앞줄에서 에러가 안나고 성공하면, 통신 성공을 화면에 띄운다
        connection = 1					#통신 상태를 1로 바꾼다(연결이 되었다고 내려준다)
    except:							#만약 에러가 나면(아두이노와 연결 시도하는 코드에서 에러가 있다면)				#다른 포트를 시도해보아라(1에서 2번포트로, 45번에서 46번으로 등등)
