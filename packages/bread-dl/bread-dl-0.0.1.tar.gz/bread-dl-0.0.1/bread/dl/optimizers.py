...
>>> from bread import dl
>>> from bread.dl.activations import ReLU
>>> from bread.dl.activations import Softmax
>>> from bread.dl.objectives import SCCE
>>> model = dl.model.Model()
>>> model.add(dl.layers.Dense(n_out=100, n_in=50, activation=ReLU()))
>>> model.add(dl.layers.Dense(n_out=200, activation=ReLU()))
>>> model.add(dl.layers.Dense(n_out=100, activation=ReLU()))
>>> model.add(dl.layers.Dense(n_out=10, activation=Softmax()))
>>> model.compile(loss=SCCE(), optimizer=dl.optimizers.SGD(lr=0.005))
    ...
    * ``param := param - learning_rate * gradient``
    ...
    def update(self, params, grads):
        for p, g in zip(params, grads):
            p -= self.lr * api_clip(g, self.clip)
