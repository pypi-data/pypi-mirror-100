...
class Recurrent(Layer):
    ...
    nb_seq : int or None
        sequent length
    init : dl.intializations.Initliazer
        init function
    inner_init : dl.intializations.Initliazer
        inner init function, between hidden to hidden
    activation : api.activations.Activation
        activation function
...
class GRU(Recurrent):
    ...
    Parameters
    ----------
    gate_activation : api.activations.Activation
        Gate activation.
    need_grad ： bool
...
class BatchLSTM(Recurrent):
    ...
    def connect_to(self, prev_layer=None):
        ...
        Parameters
        ----------
        prev_layer : api.layers.Layer or None
            Previous layer.
        AllW : numpy.array
            ===== ==== === === ===
            type   i    f   o   g
