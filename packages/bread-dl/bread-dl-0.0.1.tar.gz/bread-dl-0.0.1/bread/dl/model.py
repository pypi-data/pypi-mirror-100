...
from bread.dl.utils import get_rng
from .layers import Layer
from .optimizers import SGD
from . import optimizers
from bread.dl.utils import get_dtype
...
class Model(object):
    ...
    def compile(self, loss=SoftmaxCategoricalCrossEntropy(), optimizer=SGD()):
        ...
        for idx in range(len(self.layers) - 1):
            self.layers[idx + 1].first_layer = False
    ...
    def fit(self, X, Y, max_iter=100, batch_size=64, shuffle=True,
        ...
        while iter_idx < max_iter:
            ...
            return np.mean(train_losses), self.accuracy(train_predicts, train_targets)
    ...
    def load(self, from_path):
        raise NotImplementedError('Load model.')

    def save(self, to_path):
        raise NotImplementedError('Save model.')
