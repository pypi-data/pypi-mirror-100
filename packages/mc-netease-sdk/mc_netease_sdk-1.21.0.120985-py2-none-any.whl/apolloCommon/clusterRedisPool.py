...
def AsyncDelete(dbName, key, callback):
	...
	Args:
		dbName         str            redis db名字，名字在deploy.json中cluster_redis下配置，比如示例配置中 “cluster_redis1”
		key            str            redis中的key
...
def AsyncFuncWithKey(dbName, func, orderKey, callback, *args, **kwargs):
	...
	Args:
		dbName         str            redis db名字，名字在deploy.json中cluster_redis下配置，比如示例配置中 “cluster_redis1”
		func           function       redis异步任务，可以没有返回值。该任务和主线程会并行执行，要求任务是线程安全的。第一个参数是一个redis长连接，是一个redis连接实例，其他参数是*args
...
def InitDB(dbName, poolSize):
	# type: (, ) -> None
	"""
	   @description 初始化redis连接池，要求在MCStudio的“服务器配置”中新增cluster redis实例

	Args:
		                               @param dbName str redis实例名字，对应MCStudio中redis“实例名称”配置
		                               @param poolSize int 连接池大小,限制最大值为64,不传此参数时采用配置文件中的max_connections设置,max_connections没有设置时采用默认值16
