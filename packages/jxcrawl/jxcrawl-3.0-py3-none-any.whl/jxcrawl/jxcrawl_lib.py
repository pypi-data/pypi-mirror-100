...
stock_300_rule_dict = cnstock.get_rule(code_type='sz300') 		# 获取中国股市深市 300 创业板股票交易规则
...
def get_all_code(wait_time=cf.WAIT_TIME, source='eastmoney'):
	...
	if source.upper() == 'EASTMONEY':
		all_code_arr = get_all_code_from_eastmoney(wait_time=wait_time)
...
def is_trade_date(category='stock', market='hushen'):
	...
	if category.upper() == 'STOCK' and market.upper() == 'HUSHEN':
		...
		"""
		df = get_realtime_stock_data_async(code_arr=[shzz], index=True, source='sina')
		if df is None:
			df = get_realtime_stock_data_async(code_arr=[shzz], index=True, source='netease') 
			if df is None:
				print(jxb.get_current_time(),func_name,'由于 get_realtime_stock_data_async() 没有返回实时数据，本函数无法判断今天是否交易日，将返回 None')
				return None
		"""
		df = get_data(code_arr=[shzz], index=True)
...
def get_realtime_stock_data_async(code_arr, index=False, source='sina'):
	"""
	...
	if source.upper() == 'NETEASE':
		package_size = 1000 			# 一次拿几只股的实时数据 (新浪一次性最大支持 800只)
	...
	for i in range(xcount):
		...
		if source.upper() == 'NETEASE':
			url = get_netease_realtime_stock_url(code_arr=temp_arr, index=index) 		# 把股票 list 串接成符合新浪数据源要求的格式返回
		url_arr.append(url)
...
def get_realtime_stock_data_sync(code_arr, index=False, source='sina'):
	"""
...
def get_realtime_stock_data_sync_0(code_arr, index=False, source='sina'):
	"""
	...
	if source.upper() == 'NETEASE':
		url = get_netease_realtime_stock_url(code_arr=code_arr, index=index) 				# 把股票 list 串接成符合新浪数据源要求的格式返回
...
def get_sina_realtime_stock_data_from_page_source(page_source):
	...
	df['change'] = df['price'] - df['last_close'] 				# 计算出涨跌额
	df['percent'] = df['change'] / df['last_close'] 			# 计算出涨跌幅
...
def get_sina_realtime_usa_stock_data():
	...
	for k,page_source in enumerate(page_source_arr):
		...
		df = pd.read_json(sub_data_json, orient='records', dtype=False)
		all_df = pd.concat([all_df,df])
	...
	# --------------------------------------------
	# 这些数值列转成 float
	float_column_arr = ['last_close','open','high','low','price','change','percent','amplitude','total_value','pe']
...
def get_all_code_from_eastmoney(wait_time=cf.WAIT_TIME):
	...
	-------
	all_code_arr : list
		DESCRIPTION. 返回值是一个包含所有股票代码的 list

	"""
	all_code_arr = get_all_code_from_eastmoney1()
...
def get_all_code_from_eastmoney1():
	...
	返回值：list 形式的股票代码
	"""
	[total_page, all_code_arr] = get_total_page_from_eastmoney()
	if total_page <= 1:
		return all_code_arr
...
def get_total_page_from_eastmoney():
	...
	参数：无
	返回值：是一个 list，包含两个元素，第一个元素是总页数(int类型)，第2个元素是一个list，存放的是主力排名节首页的股票代码
	"""
	result_arr = []			# 存放总页码及首页股票代码子list
	...
	result_arr = [total_page, code_arr]
...
def get_eastmoney_main_rank(wait_time=cf.WAIT_TIME):
	...
	if all_df is not None:
		...
		# 从 all_df 中提取以下4类股票
		stock_60_df = all_df[all_df['code'].str.contains(stock_60_rule_dict['CODE_RE'])] 		# 获取 60 开头的股票行，stock_60_rule_dict['CODE_RE'] 是一个正则表达式，contains() 也能接受的，它也可以接受字符串作为参数
		stock_688_df = all_df[all_df['code'].str.contains(stock_688_rule_dict['CODE_RE'])] 		# 获取 688 开头的股票行，stock_688_rule_dict['CODE_RE'] 是一个正则表达式，contains() 也能接受的，它也可以接受字符串作为参数
		stock_00_df = all_df[all_df['code'].str.contains(stock_00_rule_dict['CODE_RE'])] 		# 获取 00 开头的股票行，stock_00_rule_dict['CODE_RE'] 是一个正则表达式，contains() 也能接受的，它也可以接受字符串作为参数
		stock_300_df = all_df[all_df['code'].str.contains(stock_300_rule_dict['CODE_RE'])] 		# 获取 300 开头的股票行，stock_300_rule_dict['CODE_RE'] 是一个正则表达式，contains() 也能接受的，它也可以接受字符串作为参数
		# 把 4 类股票合在一起
		all_df = pd.concat([stock_60_df,stock_688_df,stock_00_df,stock_300_df])
		all_df = all_df.drop_duplicates()		# 去除重复记录		
		all_df = all_df.sort_values(by=['code'], ascending=True)
		all_df = all_df.reset_index(drop=True)
