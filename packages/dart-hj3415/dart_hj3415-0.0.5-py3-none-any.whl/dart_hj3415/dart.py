...
def _make_dart_list(first_url: str, echo: bool) -> list:
    ...
    if echo:
        print(f'Extracting pages({total_page}) ', end='', flush=True)
    for i in range(total_page):
        ...
        time.sleep(1)
    if echo:
        print(f'total {len(list_raw_dict)} items..', flush=True)
...
def save_db_by_date(edate: str, db_fullpath: str, echo=False):
    df = get_df(edate=edate, echo=echo)
    if df.empty:
...
def save_db_by_corp(edate: str, db_fullpath: str, echo=False):
    ...
    print(f"Distribute each item to corp_dbs ...", flush=True)
    for i, row in df.iterrows():
        with engine.connect() as conn:
            # t+코드명의 테이블이 없으면 만든다.
            conn.execute(text(f"""CREATE TABLE IF NOT EXISTS t{row['stock_code']} (
                                                rcept_no String NOT NULL PRIMARY KEY, 
                                                rcept_dt String, 
                                                report_nm String
                                                );"""))
            try:
