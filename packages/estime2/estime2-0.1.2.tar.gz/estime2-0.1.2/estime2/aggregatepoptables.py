...
def get_Ns(
    ...
    while flag:
        if age_is_max and (not comp_in_comp_end):
            # method_use.old_neigh is ALWAYS False under this condition
            for i1 in tbl_index:
            ...
            using_old_neigh =\
                to_transfer_ages_comp[0] > to_transfer_ages_comp[-1]
            for i2 in tbl_index:
                ...
                if local_rem >= 1:
                    comp_N_local[-1] = 1 if other_comp_in_neg else -1
                    to_iterate = range(nrow - 1) if using_old_neigh \
                        else range(nrow - 2, -1, -1)
                    for j2 in to_iterate:
