...
class ParseInputData(SupportTaskBase, ABC):
    ...
    @staticmethod
    def translate_fits_file(filepath: Path) -> (fits.header.Header, np.ndarray):
        """
        Perform the spec 122 to spec 214 translation on fits headers
        Parameters
        ----------
        ...
        Returns
        -------
        the translated header in spec 214 format, the data array that was in the original fits file
        """
        hdul = fits.open(filepath)
        s214_hdulist = spec122_validator.validate_and_translate(hdul)
        return s214_hdulist[0].header, hdul[0].data
