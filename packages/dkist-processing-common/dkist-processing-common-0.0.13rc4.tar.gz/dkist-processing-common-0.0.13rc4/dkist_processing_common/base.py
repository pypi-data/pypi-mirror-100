...
class TaskBaseExt(TaskBase, ABC):
    ...
    def input_paths(self) -> Generator[Path, None, None]:
        ...
        """
        return self.input_dir.glob("*.fits")
    ...
    def write_translated_input_fits(
        self, data: np.ndarray, header: fits.header.Header, filename: str, tags: Optional[List] = []
    ):
        """
        ...
        ----------
        data: np.ndarray
            numpy array to be written to the fits file
        ...
        hdul_to_write = fits.HDUList([hdu])
        self._scratch.write_fits(data=hdul_to_write, relative_path=filename, tags=["INPUT"] + tags)
