...
class Event(Model):
    ...
    image = ImageField(upload_to=f'{settings.MEDIA_ROOT}/event')
...
class Artist(Model):
    ...
    picture = ImageField(upload_to=f'{settings.MEDIA_ROOT}/artist')
...
class Product(Model):
    ...
    title = CharField(max_length=50)
    description = TextField()
    picture = ImageField(upload_to=f'{settings.MEDIA_ROOT}/product')
...
class ExtraPicture(Model):
    reference = ForeignKey(Product, on_delete=CASCADE)
    image = ImageField(upload_to=f'{settings.MEDIA_ROOT}/product')
