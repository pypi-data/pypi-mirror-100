...
class KodexaDocumentWidget(VueTemplate):
    ...
    options = traitlets.Dict({}).tag(sync=True)
    document_family = traitlets.Dict({}).tag(sync=True)
    ...
    def __init__(self, document, document_family=document_family, width=None, height=None, mixin=None, options=None,
                 tags=None):
        ...
        if tags is None:
            tags = []
        if options is None:
            options = {'menubar': True, 'spatial': {'showPageSelector': True}, 'allowTagging': True}
        ...
        self.document_family = document_family.to_dict()
