...
class Credentials:
    ...
    def __init__(self):
        ...
        if mode:  # .venv was already read in other project
            self.host = os.environ["DB_HOST"]
            self.port = os.environ["DB_PORT"]
            self.database = os.environ["DB_DATABASE"]
            self.user = os.environ["DB_USER"]
            self.pw = os.environ["DB_PASSWORD"]

        else:  # read .venv from this project

            load_dotenv(Path(Path(__file__).parent.parent.parent / '.env'))
...
engine_url = 'postgresql://{}:{}@{}:{}/{}'.format(cred.user,
                                                  cred.pw,
                                                  cred.host,
                                                  cred.port,
                                                  cred.database)
...
def generate_insert_sql(df, table, schema):
    """generate raw insert sql"""
    cols = get_cols(df.columns)
...
def generate_insert_sql_execute_values(df, table, schema):
...
def bulk_insert(df, table, schema, con):
    ...
    with con.cursor() as cur:
        ...
        sql = generate_insert_sql_execute_values(df, table, schema)
        extras.execute_values(cur, sql, data)


def to_sql(df, table, schema):
