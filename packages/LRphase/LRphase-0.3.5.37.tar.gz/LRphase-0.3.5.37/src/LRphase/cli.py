...
def phasing(args):
    ...
    print('phasing mode')
    print(args)
    input_data = InputData.InputData(args.output_directory_path)
    ...
    for phasable_sample in input_data:
        ...
        for alignment in phasable_sample:
            phased_read = PhasedRead.PhasedRead(alignment, phasable_sample)
...
def getArgs() -> object:
    ...
    phasing_parser_output.add_argument(
        '-sep', '--output_phase_separated_bam',
        help='Use the --output_phase_separated_bam option to output the phased reads as individual bam files '
    ...
    error_analysis_parser = lrphase_subparsers.add_parser(
        'error_analysis',
        description='Tool for performing error analysis of phasing results using simulated reads and haplotype '
                    'information. '
    )
