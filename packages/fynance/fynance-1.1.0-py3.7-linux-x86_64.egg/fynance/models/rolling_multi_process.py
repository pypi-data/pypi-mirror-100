...
# @Last modified time: 2020-11-28 08:48:24
...
class _RollingBasis:
    ...
    def run(self, backtest_plot=True, backtest_kpi=True, figsize=(9, 6),
        ...
        for eval_set, test_set in self:
            ...
            if self.t > self.t0 + self.r:
                if p_print is None:
                    ...
                    p_print = Process(
                        target=self._print,
                        args=(self.t, self.i, r, self.perf_test,
                              self.perf_eval, y_perf, func,
                              backtest_plot, backtest_kpi, (q1, q2))
                    )
                    # cnxn1.send(bnn)
                    q1.put(bnn, False)
                    p_print.start()
                ...
                # elif not q2.empty():
                    ...
                    p_print.join()
                    p_print.close()
                    # _bnn.f.canvas.draw_idle()
                    p_print = None
                    # cnxn1.close()


        if p_print is not None:
            # if not q.empty():
            #    _bnn = q.get()
        ...
        # cnxn1.send(bnn)
        q1.put(bnn, False)
        self._print(self.t, self.i, r, self.perf_test, self.perf_eval,
                    y_perf, func, backtest_plot, backtest_kpi, (q1, q2))

        return self

    def _print(self, t, i, r, perf_test, perf_eval, y_perf, func,
               backtest_plot, backtest_kpi, cnxn=None):
        if cnxn is not None:
            # bnn = cnxn.recv()
            bnn = cnxn[0].get()

        bnn = BacktestNeuralNet((9, 6))

        if backtest_kpi:
            self._display_kpi(t)

        if backtest_plot:
            self._display_plot_loss(bnn, i)

            self._display_plot_perf(
                bnn, perf_test, perf_eval, y_perf, t
            )
            # if q is None:
            if True: # cnxn is None:
                bnn.f.canvas.draw()
                plt.show(block=False)
                # plt.show()

            else:
                cnxn[1].put(bnn, False)
                # cnxn.send(bnn)
                # cnxn.close()
