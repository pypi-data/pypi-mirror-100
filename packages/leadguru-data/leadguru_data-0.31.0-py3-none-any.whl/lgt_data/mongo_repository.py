...
class LeadMongoRepository(BaseMongoRepository):
    ...
    def get_list(self, skip, limit, **kwargs):
        ...
        if text:
            pipeline['$or'][0]['$and'].append({'message.message': {'$regex': text, '$options': 'i'}})
            pipeline['$or'].append({'message.profile.real_name': {'$regex': text, '$options': 'i'}})
            pipeline['$or'].append({'message.profile.title': {'$regex': text, '$options': 'i'}})

        if stop_words:
            pipeline['$or'][0]['$and'].append(
                {'message.message': {'$regex': f'^(?!.*({stop_words})).*$', '$options': 'i'}}
            )
