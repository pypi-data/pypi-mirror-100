...
class Lines:
    ...
    def n_pane_comparison(cls, t, y,
                          axis_labels, legend_labels, filename=None, where_does_this_go=None,
        ...
        for i in range(len(y)):
            ax_transient = plt.subplot2grid((1, len(y)), (0, i), rowspan=1, colspan=1)
            print(y.shape, t.shape), print(axis_labels)
            if i < (len(y) - 1):
                cls.comparison([t[i][n] for n in range(len(y[1]))], [y[i][n] for n in range(len(y[1]))],
                               ax_transient, fig,
                               y_label=axis_labels[i], zorders=zorders, colors=colors, alphas=alphas, )
            else:
                cls.comparison([t[i][n] for n in range(len(y[1]))], [y[i][n] for n in range(len(y[1]))],
                               ax_transient, fig,
                               y_label=axis_labels[i], zorders=zorders, colors=colors, alphas=alphas,
