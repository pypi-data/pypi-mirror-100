...
class attributes:
    ...
    def method_ticks(self):
        ...
        if not isinstance(self.x_custom_tick_locations, type(None)):
            ...
            self.ax.set_xticks(np.linspace(self.x_custom_tick_locations[0], self.x_custom_tick_locations[1], self.x_tick_number),
                               # fontname=self.font
                               )
        if not isinstance(self.y_custom_tick_locations, type(None)):
            # Previous version: set_yticklabels()
            self.ax.set_yticks(np.linspace(self.y_custom_tick_locations[0], self.y_custom_tick_locations[1], self.y_tick_number),
                               # fontname=self.font
                               )
        #   Custom tick labels
