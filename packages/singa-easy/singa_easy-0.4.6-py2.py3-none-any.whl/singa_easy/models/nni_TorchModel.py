...
class nni_TorchModel(SINGAEasyModel):
    """
    ...
    def get_knob_config():
        return {
            'model_class':CategoricalKnob(['nni_TorchModel']),
            # Learning parameters
            'lr': CategoricalKnob([0.0001]),  ### learning_rate nni
            'weight_decay': FixedKnob(0.0), ### 0.0001 for SGD
            'momentum':FixedKnob(0),
            'drop_rate': FixedKnob(0.0),
            'max_epochs': FixedKnob(1), ### nni
            'batch_size': CategoricalKnob([128]), ### nni
            'max_iter': FixedKnob(20),
            'optimizer': CategoricalKnob(['sgd']), ### nni
            'scratch': FixedKnob(True),

            # Data augmentation
            'max_image_size': FixedKnob(32), ### nni
            'share_params': CategoricalKnob(['SHARE_PARAMS']),
            'tag': CategoricalKnob(['relabeled']),
            'workers': FixedKnob(8),
            'seed': FixedKnob(123456), ### nni
            'scale': FixedKnob(256),
            ...
            'enable_lossrevise': FixedKnob(True),
            ...
            'mc_trials_n': FixedKnob(5),
    # }
            'hidden_size': FixedKnob(512), ### nni
            'momentum':FixedKnob(0), ### nni 0.5
        ...
        for epoch in range(1, self._knobs.get("max_epochs") + 1):
            ...
            for batch_idx, (raw_indices, traindata,
                ...
                if self._knobs.get("enable_model_slicing"):
                    ...
                    # Slicng_timing.append(slicing_timing)
                else:
                    ...
                    trainloss.backward()
                if self._knobs.get("enable_gm_prior_regularization"):
                    # print ('****** apply_GM_regularizer_constraint****\n ******** dataset.classes ******\n', dataset.classes)
                    # print('************  dataset.size *********\n', dataset.size)
            ...
            try:
                ...
                batch_accuracy_mean = np.mean(batch_accuracy)
                print ('Epoch val Average loss: {:.4f}, Accuracy: {}. epoch val batch size{} \n'.format(
    ...
    def dump_parameters(self):
        ...
        with tempfile.NamedTemporaryFile() as tmp:
            ...
            # print ('*********** TorchModel dump_parameters to {}************'.format(tmp.name))
