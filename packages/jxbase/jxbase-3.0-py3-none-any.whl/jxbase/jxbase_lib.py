...
def change_path_style(xpath=None, style=cf.UNIX_STYLE_ARR[0]):
	"""
	功能说明：转换路径风格
	参数：xpath: 为待转换的路径； style： 为目标风格，接受值：'dos','windows','linux','unix', 以及他们的大写，含意与小写相同。默认 unix（linux）风格
	返回值：转换风格后的路径
	"""
	if xpath is None:
		print('没有输入路径')
...
def convert_df_type(df, xtype=float, column_arr=None):
	"""
	功能说明：对传入的 df ，根据传入的字段 column_arr （如果有的话，要是没有的话，则对 df 中所有字段转换），
...
def df_to_list(df):
	"""
	功能说明：将 df 转成二维数组（不包括列名）
	参数：df: pandas的 DataFrame数据
	返回值：二维数组

	"""
	result_arr = df.values.tolist()
...
def df_to_table(df, align='center', color_flag=False, fore_color1='black', fore_color2='black', bgcolor1="#F5F5DC", bgcolor2="#A7EEFB"):
	"""
	功能说明：将 df 转成 html 的 table 形式，并以字符串形式返回，主要是为了发电子邮件用
	参数：df:待转换的 pandas DataFrame数据；align:对齐方式，可取值为 'left','center','right'；color_flag:要不要用前景色背景色装饰表格，取值True或False；
...
def df_to_webpage(df, align='center', color_flag=False, fore_color1='black', fore_color2='black', bgcolor1="#F5F5DC", bgcolor2="#A7EEFB"):
	"""
	...
	html_table = df_to_table(df=df, align=align, color_flag=color_flag, fore_color1=fore_color1, fore_color2=fore_color2, bgcolor1=bgcolor1, bgcolor2=bgcolor2)
...
def get_current_time(xtime=time.time(), time_format=cf.DATE_TIME_FORMAT2):
	"""
	功能说明：根据传入的时间戳，取出时间返回
	参数：xtime: 是时间戳，如time.time() 返回的结果
...
def get_rsa_key(bit=1024):
	...
	返回值：list，共有两个元素，元素类型都是字节流，第1个元素是 public key, 第2个元素是 private key
...
def kill_app(app_name=None, process_id=None):
	...
	参数：app_name 指向程序名；process_id 指向进程 id；这两个参数只需传入一个即可。
		若两个都传入，只优先使用 app_name，若采用 app_name关闭失败，则再采用 process_id 来关闭。
...
def make_header():
	...
	header = {
		...
		'User-Agent': ua.random 			# 使用随机请求头（ua.random 返回的是字符串形式）
		}
	return header
...
def retry_command(command, para='', wait_time=None, retry_count=10, auto_wait=True):
	...
	参数：command: 传入的命令（对象）；para: 字符串形式的参数链，必须完整；wait_time: 表示两次尝试之间的等待时间；retry_count: 尝试执行次数; 
...
def security_check(ip):
	"""
	功能说明：本函数获取本机 公网 ip ，然后对公网ip做 hash (sha512)操作，
		然后和传入的 ip （应事先已用 sha512 hash过）比较，以判断程序是否运行在传入的 ip 电脑上，
		若不是，则返回 False 给上层代码，以决定是否退出，这样可防止程序被非法运行在指定 IP 以外的电脑上。
	参数：ip: 已用 sha512 hash过的 ip,
	"""
	if ip is None:
		print('请输入 用 sha512 hash 过的 ip, 通常调用 get_hash() 默认可以对 ip 进行 sha512 哈希')
		return False
...
def sort_df_column(df, column_arr):
	...
	返回值：列排序后的 df
	"""
	df_column_arr = df.columns.tolist() 			# 获取 df 的列名称
	position = 0
