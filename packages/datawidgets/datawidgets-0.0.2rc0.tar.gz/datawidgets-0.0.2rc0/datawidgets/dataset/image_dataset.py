...
class ImageClassificationDataset(
    ...
    def get_modified_df(self):
        ...
        for fname, datapoint in self.datapoints.items():
            ...
            if item.is_modified or item.is_completed:
                rows.append([fname, labels, note, item.is_modified, item.is_completed])
    ...
    def setup_view(self, global_callbacks=[]):
        ...
        delete_button = self.generate_delete_button(callbacks)
        (
            self.mark_completed_button,
            self.mark_completed_toggle,
        ) = self.generate_mark_completed_buttons([self.update_grid, self.update_info])
        ...
        status_controls = HBox(
            [delete_button, self.mark_completed_button, self.mark_completed_toggle],
        )
