        ...
        if jwt is not None:
            ...
            if 'AWS_SESSION_TOKEN' in os.environ and not has_expired(os.environ['AWS_SESSION_TOKEN']):
                ...
                if response.status_code == 200:
                    sts_cred = json.dumps(xmltodict.parse(response.content))
                    app_log.info(sts_cred)
                    os.environ['AWS_ACCESS_KEY_ID'] = sts_cred['AccessKeyId']
                    os.environ['AWS_SECRET_ACCESS_KEY'] = sts_cred['SecretAccessKey']
                    os.environ['AWS_SESSION_TOKEN'] = sts_cred['SessionToken']
