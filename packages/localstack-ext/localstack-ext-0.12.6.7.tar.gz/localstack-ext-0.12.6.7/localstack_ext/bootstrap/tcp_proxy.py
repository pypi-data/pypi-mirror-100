...
class AttrDict(CBbQJ):
 def __init__(self,*args,**kwargs):
  CBbQi(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and CBbQE(socket,'IPV6_V6ONLY'):
...
class Half(CBbQF):
 def __init__(self,opt,sock,addr,CBbQK):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.CBbQK=CBbQK
  self.name='peer' if self.CBbQK else 'client'
  self.queue=[]
  self.dest=CBbQW
  self.err=CBbQW
  self.ready=CBbQD
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':CBbQD,'bind_port':2223,'addr':'192.168.43.191','port':22}
 opt={'ip6':CBbQD,'bind_port':48429,'addr':'128.0.100.3','port':22}
 opt={'ip6':CBbQD,'port':54237,'bind_addr':'128.0.100.2','bind_port':22}
 opt={'ip6':CBbQD,'port':54237,'bind_addr':'192.168.100.3','bind_port':22}
 server_loop(opt)
