	...
	public:
		...
		*/
		RECompGP(const den_mat_t& coords,
			...
			this->has_Z_ = false;
			if (cov_fct == "wendland") {
				taper_mu = (1. + coords.cols()) / 2. + shape;
			}
...
#pragma omp for schedule(static)
				...
				if (cov_function_->cov_fct_type_ == "wendland") {
					CalculateDistances(coords_, coords_, true, cov_function_->taper_range_, dist);
				}
				else {
		...
		*/
		RECompGP(std::shared_ptr<T_mat> dist,
			...
			this->is_rand_coef_ = true;
			this->has_Z_ = true;
			cov_function_ = std::unique_ptr<CovFunction>(new CovFunction(cov_fct, shape));//CHANGE//DELETE
			this->num_cov_par_ = cov_function_->num_cov_par_;
		...
		*/
		void FindInitCovPar(vec_t& pars) const override {
			...
			if (cov_function_->cov_fct_type_ != "wendland") {
				double mean_dist = 0;
				if (!dist_saved_) {//Calculate distances (of a subsample) in case they have not been calculated (for the Vecchia approximation)
					int num_coord = (int)coords_.rows();
					den_mat_t dist;
					int num_data_find_init = (num_coord > 1000) ? 1000 : num_coord;//limit to maximally 1000 to save computational time
					if (num_data_find_init < num_coord) {
						std::mt19937 gen(0); //Standard mersenne_twister_engine seeded with 0
						std::uniform_int_distribution<> dis(0, num_coord - 1);

						std::vector<int> sample_ind(num_data_find_init);
					...
					//TODO: also do use subsample for large data//DELETE
		...
		*/
		void AddPredCovMatrices(const den_mat_t& coords,
			...
			if (has_duplicates) {
				if (cov_function_->cov_fct_type_ == "wendland") {
					CalculateDistances(coords, coords_pred_unique, false, cov_function_->taper_range_, cross_dist);
				}
				else {
				...
				if (cov_function_->cov_fct_type_ == "wendland") {
					CalculateDistances(coords, coords_pred, false, cov_function_->taper_range_, cross_dist);
				}
				else {
			...
			//DELETE
			//T_mat cross_dist((int)uniques_pred.size(), coords.rows());
			//cross_dist.setZero();
			//for (int i = 0; i < coords.rows(); ++i) {
			//	for (int j = 0; j < (int)uniques_pred.size(); ++j) {
			//		if (has_duplicates) {
			//			cross_dist.coeffRef(j, i) = (coords.row(i) - coords_pred_unique.row(j)).lpNorm<2>();
			//		}
			//		else {
			//			cross_dist.coeffRef(j, i) = (coords.row(i) - coords_pred.row(j)).lpNorm<2>();
			//		}
			//	}
			//}
			...
			if (predict_cov_mat) {
				T_mat dist;
				if (cov_function_->cov_fct_type_ == "wendland") {
					CalculateDistances(coords_pred, coords_pred, true, cov_function_->taper_range_, dist);
				}
				else {
