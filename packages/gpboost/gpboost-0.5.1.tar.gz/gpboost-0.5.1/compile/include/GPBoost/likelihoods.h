...
#pragma omp parallel for schedule(static)
			...
			for (it = 0; it < MAXIT_MODE_NEWTON_; ++it) {
				...
				//chol_fact_SigmaI_plus_ZtWZ_grouped_.compute(SigmaI_plus_ZtWZ);//DELETE
...
#pragma omp parallel for schedule(static)
			...
			for (it = 0; it < MAXIT_MODE_NEWTON_; ++it) {
				...
				//Log::REInfo("TIME for factorization: %g", el_time);// Only for debugging
...
#pragma omp parallel for schedule(static)
			...
			SigmaI_plus_W.makeCompressed();
			chol_fact_SigmaI_plus_ZtWZ_vecchia_.factorize(SigmaI_plus_W);
			//chol_fact_SigmaI_plus_ZtWZ_vecchia_.compute(SigmaI_plus_W);//DELETE
			...
			el_time = (double)(std::chrono::duration_cast<std::chrono::microseconds>(end - beginall).count()) / 1000000.;// Only for debugging
			//Log::REInfo("TOTAL TIME for FindModePostRandEffCalcMLLVecchia: %g", el_time);// Only for debugging
...
#pragma omp parallel for schedule(static)
			...
			Id.setIdentity();
			//sp_mat_t SigmaI_plus_ZtWZ_inv = chol_fact_SigmaI_plus_ZtWZ_grouped_.solve(Id);//DELETE
...
#pragma omp parallel for schedule(static)
			...
			//eigen_sp_Lower_sp_RHS_cs_solve(chol_fact_SigmaI_plus_ZtWZ_vecchia_L, P_Id, L_inv, true);//slower when using cs_solve
			sp_mat_t SigmaI_plus_W_inv = L_inv.transpose() * L_inv;

			end = std::chrono::steady_clock::now();//DELETE
			el_time = (double)(std::chrono::duration_cast<std::chrono::microseconds>(end - begin).count()) / 1000000.;// Only for debugging
			//Log::REInfo("TIME for SigmaI_plus_W_inv: %g", el_time);// Only for debugging
			...
			el_time = (double)(std::chrono::duration_cast<std::chrono::microseconds>(end - beginall).count()) / 1000000.;// Only for debugging
			//Log::REInfo("TOTAL TIME for CalcGradNegMargLikelihoodLAApproxVecchia: %g", el_time);// Only for debugging
		...
		*/
		...
		void PredictLAApproxGroupedRE(const double* y_data,
			...
			if (calc_pred_cov || calc_pred_var) {
				...
				Maux = Zt * second_deriv_neg_ll_.asDiagonal() * Cross_Cov.transpose();//DELETE
