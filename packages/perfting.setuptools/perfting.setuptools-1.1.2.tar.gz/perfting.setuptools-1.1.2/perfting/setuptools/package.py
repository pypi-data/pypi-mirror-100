...
from perfting.setuptools.extension import Extension
...
class Package(attriclass(
    ...
    class Description(attriclass(
            ...
            type = "long_description_content_type")):
        ...
        def __init__(self,
            ...
            self.type = None
            if long:
                with open(long, "r") as fp:
                    self.long = fp.read()
                    self.type = self.mimetypes[os.path.basename(fp.name).split(".")[-1]]
    ...
    class Extensions(attriclass(
            extensions = "ext_modules")):

        def __init__(self, *extensions: Extension) -> None:
            self.extensions = [setuptools.Extension(**extension.extract()) for extension in extensions]
