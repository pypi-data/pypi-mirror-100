...
class CVStudio(object):
    @staticmethod
    def interactive(token=None, base_url=None):
        if base_url == None:
        ...
        if token is None:
            ...
            token = input("Enter your CV Studio token: ")
    ...
    def __init__(self, interactive=False, token=None, ibm_api_key_id=None, base_url=None):
        if base_url == None:
    ...
    def getDataset(self, transform=None):
        dataset=getDataset(annotations=self.get_annotations(), bucket_name=self.bucket, transform=transform)
        return dataset
...
def getDataset(annotations, bucket_name, transform):
    ...
    dataset=Dataset(annotations=annotations, bucket_name=bucket_name, transform=transform)
    return dataset