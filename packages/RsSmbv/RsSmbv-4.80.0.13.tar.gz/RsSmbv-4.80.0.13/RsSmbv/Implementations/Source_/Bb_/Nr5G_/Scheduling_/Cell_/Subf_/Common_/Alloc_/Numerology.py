...
class Numerology:
	...
	def get(self, channel=repcap.Channel.Default, stream=repcap.Stream.Default, allocation=repcap.Allocation.Default) -> enums.Numerology:
		"""SCPI: [SOURce<HW>]:BB:NR5G:SCHed:CELL<CH>:SUBF<ST>:COMMon:ALLoc<DIR>:NUMerology \n
		Snippet: value: enums.Numerology = driver.source.bb.nr5G.scheduling.cell.subf.common.alloc.numerology.get(channel = repcap.Channel.Default, stream = repcap.Stream.Default, allocation = repcap.Allocation.Default) \n
		...
		response = self._core.io.query_str(f'SOURce<HwInstance>:BB:NR5G:SCHed:CELL{channel_cmd_val}:SUBF{stream_cmd_val}:COMMon:ALLoc{allocation_cmd_val}:NUMerology?')
