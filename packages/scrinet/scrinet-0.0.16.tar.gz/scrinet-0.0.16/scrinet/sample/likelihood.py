...
class GwPriors(NamedTuple):
    ...
    distance: Any = tfd.Uniform(1, 8.)
    m_tot: Any = tfd.Uniform(60., 100.)
    mass_ratio: Any = tfd.Uniform(1, 8.)
    chi_1: Any = tfd.Uniform(-1., 1.)
    chi_2: Any = tfd.Uniform(-1, 1.)
    theta: Any = tfd.Uniform(0, 2 * np.pi)
    phi: Any = tfd.Uniform(0, 2 * np.pi)
    phase_shift: Any = tfd.Uniform(0, 2 * np.pi)
    ra: Any = tfd.Uniform(0, 2 * np.pi)
    dec: Any = tfd.Uniform(0, 2 * np.pi)
    psi: Any = tfd.Uniform(0, 2 * np.pi)
    time: Any = tfd.Uniform(2, 4.)  # can make this very restrictive
