...
def generate_surrogate_hp_hc(
    ...
    # window_test = make_rect_window_tensor(num_t_int, [ab[0, i] for i in range(n_batch)])
    ...
    hp_rolled = apply_roll(hplus * window, -tf.reshape(index_peak_amp, (n_batch,)))
    hc_rolled = apply_roll(hcross * window, -tf.reshape(index_peak_amp, (n_batch,)))
...
def convert_tfp_chains_to_arviz_object(chains, param_names):
    """
    ...
    samples_dict = {key: samples_right_shape[:, :, i] for i, key in enumerate(param_names)}
