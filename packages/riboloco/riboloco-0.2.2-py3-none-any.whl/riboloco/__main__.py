...
def convert_bed_to_single_codon_res(args, df, fasta_d, ref_APEs, precalculated_offsets,
    ...
    while min(done_dict.values()) == 0:  # some aren't done
                ...
                if this_type in precalculated_offsets.keys():  # this was in reference so best already calculated
                    ...
                    if not args.use_KL:
                        ...
                        if not args.no_iterative_improvement:
                            ...
                            ref_APEs = append_to_ref(ref_APEs, these_ratios, best_nt_offset, this_type)
        ...
        if assigned:
            kl_d_d[this_type] = save_KL(these_fractions, this_background, this_type, best_nt_offsets[this_type], args,
                                        save=args.save_KL)
    ...
    write_files(df, full_d, best_nt_offsets, args, all_r_values, oof_d, frameness_d, fasta_d, counts_df,
...
def plot_KL_and_r(kl_d_d, pdf_pages, title, vmax=1, square=False):
...
def write_files(df, full_d, best_nt_offsets, args, r_values, oof_d, frameness_d, fasta_d, counts_df,
                total_counts_cds, kl_d_d):
    print("Writing files...")
