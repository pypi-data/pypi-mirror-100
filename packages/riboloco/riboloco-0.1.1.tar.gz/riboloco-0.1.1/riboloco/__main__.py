...
# python3 python_findkl.py -s test_data/br_samples.csv -d snakemake_output/results/transcriptome_dedup/ -i /camp/lab/ulej/home/users/wilkino/genomes/mouse/longest_proteincoding_transcript_mm_details.txt -f /camp/lab/ulej/home/users/wilkino/genomes/mouse/gencode_M22/longest_gencode22.fa -o test_ref_ape.csv -fr
# python3 python_findkl.py -os snakemake_output/results/transcriptome_dedup/Br_Mon_1p5.Aligned.toTranscriptome.out.bam.bed.gz -i /camp/lab/ulej/home/users/wilkino/genomes/mouse/longest_proteincoding_transcript_mm_details.txt -f /camp/lab/ulej/home/users/wilkino/genomes/mouse/gencode_M22/longest_gencode22.fa -o test_os_mode -r test_ref_ape.csv

# for marija samples
# python3 kl.py -i /camp/lab/ulej/home/users/wilkino/genomes/hs/gencode_29/longest_proteincoding_transcript_hs_details.txt -f /camp/lab/ulej/home/users/wilkino/genomes/hs/gencode_29/gencodev29transcriptome_fixed_longestonly.fasta -o test_output -s samples.csv -d snakemake_output/results/transcriptome_dedup/ -fr
# python3 kl.py -i /camp/lab/ulej/home/users/wilkino/genomes/hs/gencode_29/longest_proteincoding_transcript_hs_details.txt -f /camp/lab/ulej/home/users/wilkino/genomes/hs/gencode_29/gencodev29transcriptome_fixed_longestonly.fasta -o test_output -os snakemake_output/results/transcriptome_dedup/Mon_C1_mn.Aligned.toTranscriptome.out.bam.bed.gz -r test_ref_ape.csv
...
def read_fasta(args):
    ...
    fasta = {}
    with open(args.transcript_fasta, 'r') as file:
        for line in file:
            if line.rstrip()[0:1] == ">":
...
def find_best_offsets(these_ratios, this_type, args, ref_APEs):
    ...
    best_nt_offset = max(r_values, key=r_values.get)
...
def save_KL(fractions, background, this_type, offset, args):
    """
...
def convert_bed_to_single_codon_res(args, df, fasta_d, ref_APEs, calculated_offsets,
                                    total_counts_cds, info, orf_df, oof_distance=500):
    """
    ...
    calculated_offsets = dictionary of offsets that were calculated already
    """
    ...
    for this_type in types["read_type"]:
        ...
        print("\nAttempting to find match for " + this_type)
        ...
        bad, best_nt_offset, r_values = find_best_offsets(these_ratios=these_ratios, this_type=this_type,
        ...
        if this_type in args.conversion_types.keys():
            ...
            if bad:
                print("Warning! Assignment for " + this_type + " was poor or ambiguous")
                if this_type in out_of_frame:
                    print("This may be due to the large number of predicted out of frame ribosomes for this footprint!"
                          " This means that assignment assuming in frame codons may not be possible!")
                print("You may wish to manually verify this offset, then manually correct "
                      "or use an extra reference.")
                print("See documentation for advice on how to improve matches.")
            else:  # if good
                assigned = True
                if this_type in calculated_offsets.keys():
                    best_nt_offsets[this_type] = calculated_offsets[this_type]
                    best_r_values[this_type] = r_values[calculated_offsets[this_type]]
                    print("Offset assigned: " + str(calculated_offsets[this_type]))
                else:
                    if not args.use_KL:
        ...
        if assigned and args.save_KL:
            save_KL(these_fractions, this_background, this_type, best_nt_offsets[this_type], args)
    ...
    # df = df[df["read_type"].isin(best_nt_offsets.keys())]
    ...
    write_files(df, full_d, best_nt_offsets, args, all_r_values, oof_d, frameness_d, fasta_d)
...
def write_files(df, full_d, best_nt_offsets, args, r_values, oof_d, frameness_d, fasta_d):
    print("Writing files...")

    df2 = df[df["read_type"].isin(best_nt_offsets.keys())]

    if args.plot_graphs:
        generate_plots(full_d, best_nt_offsets)
    ...
    write_combined_bedgraph(df2, best_nt_offsets, args, fasta_d)
    print("Complete!")
...
def write_combined_bedgraph(df, best_nt_offsets, args, fasta_d):
    ...
    if args.save_KL:
        ...
        save_KL(fractions=fractions, background=background, this_type="final", offset="final", args=args)
...
def generate_plots(full_d, best_aa_offsets):
    """
    ...
    """
    matplotlib.use('agg')  # necessary for non-interactive session with seaborn

    # First, work out what types we have
    types = best_aa_offsets.keys()
    ...
    for this_type in types:
        ...
        for aa_offset, e in d.items():  # e is the dictionary of codon enrichments
            rel_offset = aa_offset - best_aa_offsets[this_type]
            this_dict_for_df[rel_offset] = [np.log2(a + 0.00001) for a in
        ...
        p.set_yticklabels(this_df["codon"], size=8)
        for item in p.get_yticklabels():
            item.set_rotation(0)

        # save
        dir_path = os.path.dirname(os.path.realpath(__file__))
        p.get_figure().savefig(dir_path + '/test_heatmap_' + this_type + '.pdf')
...
def main():
    ...
    parser.add_argument("-ar", "--min_abundance_ref", required=False, type=float, default=0.1,
                        help="Minimum fraction of total reads that a read type must represent for calculation of "
                             ...
                             "(10%). Warning - using low values may promote inclusion of reads which are primarily "
                             "out of frame. Recommended to keep above 5%. Read fractions are calculated for reads "
    ...
    parser.add_argument("--min_offset", type=int, default=3, required=False,
                        help="The miniumum offset length from the 3' end of the E site. (Not the A site) "
                             "Length is measured in nt. Default = 3")
    parser.add_argument("--max_offset", type=int, default=-22, required=False,
    ...
    parser.add_argument("--frameness_ratio", type=float, default=1.2, required=False,
                        help="The level of enrichment near the start codon versus downstream for a read type to be "
                             ...
                             "Default = 1.2 i.e. 20%")
    parser.add_argument("-orf", "--orf_search", action="store_true", default=False,
                        help="Assess whether reads are out of frame by looking for enrichment of each read type "
                             "at positions which have alternative reading frames. The search for ORFs takes "
                             "quite a long time, so this option is disabled by default.")
    parser.add_argument("-mm", "--mismatches", action="store_true", default=False,
                        help="Whether to consider first nt mismatches. Only available when a bed file is provided")
    ...
    parser.add_argument("--save_orfs", action="store_true", default=False, help="Save the file of all detected "
                                                                                "ORFs. Can be used for downstream analysis, or can be provided to save time using --orf_file argument.")
    ...
    parser.add_argument("-op", "--oof_plot", action="store_true", default=False,
                        help="Save a heatmap and csv with raw data of out of frame (oof) enrichment across the cds")
    ...
    fasta_d = read_fasta(args)
    info = read_info_file(args)
    ...
    if args.save_orfs:
        ...
        if args.reference == "None":
            ref_APEs, calculated_offsets = find_APE_of_reference(args, df, fasta_d, total_counts_cds)
        ...
        if not args.generate_reference:
            ...
            if args.orf_search or args.oof_plot:
                if args.orf_file == "none_provided":
                    print("Searching for all ORFs")
                    orf_df = find_all_orfs(fasta_d, info)  # , max_tx=1000)
                else:
                    print("Reading orf file provided")
                    orf_df = pd.read_csv(args.orf_file)
            else:
                orf_df = "None"

            if args.oof_plot:
                gen_oof_plot(df=df, orf_df=orf_df, start=args.oof_plot_start, end=args.oof_plot_end,
                             stride=args.oof_plot_stride, output=args.output,
            ...
            convert_bed_to_single_codon_res(args=args, df=df, fasta_d=fasta_d, ref_APEs=ref_APEs,
                                            calculated_offsets=calculated_offsets, total_counts_cds=total_counts_cds,
                                            info=info, orf_df=orf_df)
